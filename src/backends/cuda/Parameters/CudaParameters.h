 ...
 * @details Class responsible for selecting a CUDA device, block and grid
 * dimensions, etc. \n The class can only be constructed from inside Parameters
 * and there mustn't be more than 1 instance in the code.
 */
class CudaParameters {
 public:
   ...
   * @throw std::runtime_error - If the GPU chosen is not supported (i.e., the
   * code was not compiled for its architecture).
   */
 ...
 private:
  ...
  /**
   * @brief Check whether the CUDA driver version installed is sufficient for
   * the code.
   ...
   * @return true - if we can run the code, the minimum SM model is 3.0
   * (Kepler).
   */
