 ...
 *
 * @copyright Copyright (C) 2015 - 2020 SC\@FIT Research Group, Brno University
...
CudaParameters::CudaParameters()
    : mDeviceIdx(kDefaultDeviceIdx),
      ...
      mDeviceProperties() {}  // end of CudaParameters
//----------------------------------------------------------------------------------------------------------------------
...
/**
 * Select cuda device for execution. If no device is specified, the first free
 * is chosen. The routine also checks whether the CUDA runtime and driver
 * version match and whether the GPU is supported by the code. If there is no
...
void CudaParameters::selectDevice(const int deviceIdx) {
  ...
  {
    // Check if the specified device is acceptable - not busy, input parameter
    // not out of bounds
    if ((mDeviceIdx > nDevices - 1) || (mDeviceIdx < 0)) {
      throw std::runtime_error(Logger::formatMessage(kErrFmtBadDeviceIndex,
                                                     mDeviceIdx, nDevices - 1));
    }
    ...
    if ((lastError != cudaSuccess) || (!cudaCodeVersionOK)) {
      ...
      throw std::runtime_error(
          Logger::formatMessage(kErrFmtDeviceIsBusy, mDeviceIdx));
    }
  }

  // Read the device that was allocated. No device synchronization can be used
  // for error handling.
  cudaCheckErrorsNoSync(cudaGetDevice(&mDeviceIdx));
  cudaCheckErrorsNoSync(cudaGetLastError());

  // Reset the device to be able to set the flags. No device synchronization can
  // be used for error handling.
  cudaCheckErrorsNoSync(cudaDeviceReset());
  cudaCheckErrorsNoSync(cudaGetLastError());

  // Enable mapped memory. No device synchronization can be used for error
  // handling.
  ...
  cudaCheckErrorsNoSync(
      cudaGetDeviceProperties(&mDeviceProperties, mDeviceIdx));

  // Check the GPU version. No device synchronization can be used for error
  // handling.
  if (!checkCudaCodeVersion()) {
    throw std::runtime_error(
        Logger::formatMessage(kErrFmtDeviceNotSupported, mDeviceIdx));
  }
}  // end of selectDevice
//----------------------------------------------------------------------------------------------------------------------

/**
 * Based on the dimension sizes, sensors masks, and the GPU architecture,
 * adequate CUDA kernel configurations are selected. \n
 *
 * @li <b>1D solver kernels</b> have blocks of size 256 threads and at least 8
 * times more blocks than SM processors. This gives us full potential on all
 * known GPUs and still not compromising the maximum number of blocks and
 * threads. \n
 *
 * @li <b> Transposition kernels </b> work by processing tiles of 32x32 by 4
 * warps. Every block is responsible for one 2D slab. Block size for the
 * transposition kernels (only 128 threads). \n
 *
 * @li <b> Streaming kernels </b>  always use 1D kernels of 256 threads and at
 * least 8 times more blocks than SM processors. The size of the grid is only
 * tuned for linear sensor mask, since in this execution phase, we don't know
...
void CudaParameters::setKernelConfiguration() {
  ...
  if ((size_t(mSolverGridSize1D) * size_t(mSolverBlockSize1D)) >
      fullDims.nElements()) {
    mSolverGridSize1D =
        int((fullDims.nElements() + size_t(mSolverBlockSize1D) - 1) /
            size_t(mSolverBlockSize1D));
  }

  // Transposition kernels.
  mSolverTransposeBlockSize = dim3(32, 4, 1);
  // Grid size for the transposition kernels
  mSolverTransposeGirdSize =
      dim3(mDeviceProperties.multiProcessorCount * 16, 1, 1);
  ...
  if (params.getSensorMaskType() == Parameters::SensorMaskType::kIndex) {
    // The sensor mask is smaller than 2048 * SMs than use a smaller number of
    // blocks
    if ((size_t(mSamplerGridSize1D) * size_t(mSamplerBlockSize1D)) >
        params.getSensorMaskIndexSize()) {
      mSamplerGridSize1D = int(
          (params.getSensorMaskIndexSize() + size_t(mSamplerBlockSize1D) - 1) /
...
void CudaParameters::setUpDeviceConstants() const {
  ...
  constantsToTransfer.nElements =
      static_cast<unsigned int>(fullDimSizes.nElements());
  ...
  constantsToTransfer.nElementsComplex =
      static_cast<unsigned int>(reducedDimSizes.nElements());
  ...
  // Source masks
  constantsToTransfer.presureSourceSize =
      static_cast<unsigned int>(params.getPressureSourceIndexSize());
  constantsToTransfer.presureSourceMode = params.getPressureSourceMode();
  constantsToTransfer.presureSourceMany =
      static_cast<unsigned int>(params.getPressureSourceMany());

  constantsToTransfer.velocitySourceSize =
      static_cast<unsigned int>(params.getVelocitySourceIndexSize());
  constantsToTransfer.velocitySourceMode = params.getVelocitySourceMode();
  constantsToTransfer.velocitySourceMany =
      static_cast<unsigned int>(params.getVelocitySourceMany());
...
void CudaParameters::checkCudaVersion() {
  ...
  if (cudaDriverVersion < cudaRuntimeVersion) {
    throw std::runtime_error(Logger::formatMessage(
        kErrFmtInsufficientCudaDriver, cudaRuntimeVersion / 1000,
        (cudaRuntimeVersion % 100) / 10, cudaDriverVersion / 1000,
        (cudaDriverVersion % 100) / 10));
  }
...
bool CudaParameters::checkCudaCodeVersion() {
  return (SolverCudaKernels<Parameters::SimulationDimension::k2D, true, true,
                            true, true>::getCudaCodeVersion() >= 30);
