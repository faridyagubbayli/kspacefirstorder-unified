 ...
 *
 * @brief     The header file with CUDA utility functions. These routines are to
 * be inlined.
 *
...
//--------------------------------------------------------------------------------------------------------------------//
/**
 * This variable holds necessary simulation constants in the CUDA GPU memory.
...
/**
 * @brief  Get x-stride for 3D CUDA block (for processing multiple grid points
 * by a single thread).
...
inline __device__ dim3 getComplex3DCoords(const unsigned int i) {
  return dim3(
      i % cudaDeviceConstants.nxComplex,
...
inline __device__ dim3 getComplex2DCoords(const unsigned int i) {
  return dim3(i % cudaDeviceConstants.nxComplex,
              i / cudaDeviceConstants.nxComplex, 1);
