 ...
 *
 * @brief     The implementation file of cuda kernels used for data sampling
 * (output streams).
 *
...
/**
 * CUDA kernel to sample data based on index sensor mask. The operator is given
...
__global__ void cudaSampleIndex(float* samplingBuffer, const float* sourceData,
  ...
  for (auto i = getIndex(); i < nSamples; i += getStride()) {
    switch (reduceOp) {
      ...
      case BaseOutputStream::ReduceOperator::kRms: {
        samplingBuffer[i] +=
            (sourceData[sensorData[i]] * sourceData[sensorData[i]]);
        break;
      }
...
/**
 * Sample the source matrix using the index sensor mask and store data in
 * buffer.
 */
template <BaseOutputStream::ReduceOperator reduceOp>
void OutputStreamsCudaKernels::sampleIndex(float* samplingBuffer,
                                           ...
                                           const size_t* sensorData,
                                           const size_t nSamples) {
  cudaSampleIndex<reduceOp><<<getSamplerGridSize(), getSamplerBlockSize()>>>(
...
template void
OutputStreamsCudaKernels::sampleIndex<BaseOutputStream::ReduceOperator::kNone>(
    float* samplingBuffer, const float* sourceData, const size_t* sensorData,
    const size_t nSamples);
/// Sample the source matrix using the index sensor mask, take the root mean
/// square.
template void
OutputStreamsCudaKernels::sampleIndex<BaseOutputStream::ReduceOperator::kRms>(
    float* samplingBuffer, const float* sourceData, const size_t* sensorData,
    const size_t nSamples);
/// Sample the source matrix using the index sensor mask, take the maximum.
template void
OutputStreamsCudaKernels::sampleIndex<BaseOutputStream::ReduceOperator::kMax>(
    float* samplingBuffer, const float* sourceData, const size_t* sensorData,
    const size_t nSamples);
/// Sample the source matrix using the index sensor mask, take the minimum.
template void
OutputStreamsCudaKernels::sampleIndex<BaseOutputStream::ReduceOperator::kMin>(
    float* samplingBuffer, const float* sourceData, const size_t* sensorData,
    const size_t nSamples);
...
/**
 * Transform 3D coordinates within the cuboid into 1D coordinates within the
...
inline __device__ size_t transformCoordinates(const size_t cuboidIdx,
  ...
  return (globalPosition.z * matrixSize.x * matrixSize.y +
          globalPosition.y * matrixSize.x + globalPosition.x);
}  // end of transformCoordinates
//----------------------------------------------------------------------------------------------------------------------

/**
 * CUDA kernel to sample data inside one cuboid, operation is selected by a
 template parameter.
 ...
 */
template <BaseOutputStream::ReduceOperator reduceOp>
__global__ void cudaSampleCuboid(float* samplingBuffer, const float* sourceData,
                                 const dim3 topLeftCorner,
                                 const dim3 bottomRightCorner,
                                 const dim3 matrixSize, const size_t nSamples) {
  for (auto i = getIndex(); i < nSamples; i += getStride()) {
    auto Position =
        transformCoordinates(i, topLeftCorner, bottomRightCorner, matrixSize);
    switch (reduceOp) {
      case BaseOutputStream::ReduceOperator::kNone: {
...
/**
 * Sample data inside one cuboid and store it to buffer. The operation is given
...
void OutputStreamsCudaKernels::sampleCuboid(float* samplingBuffer,
                                            ...
                                            const dim3 matrixSize,
                                            const size_t nSamples) {
  cudaSampleCuboid<reduceOp><<<getSamplerGridSize(), getSamplerBlockSize()>>>(
      samplingBuffer, sourceData, topLeftCorner, bottomRightCorner, matrixSize,
      nSamples);
  // Check for errors
  cudaCheckErrors(cudaGetLastError());
}  // end of sampleCuboid
...
template void
OutputStreamsCudaKernels::sampleCuboid<BaseOutputStream::ReduceOperator::kNone>(
    float* samplingBuffer, const float* sourceData, const dim3 topLeftCorner,
    const dim3 bottomRightCorner, const dim3 matrixSize, const size_t nSamples);
/// Sample data inside one cuboid and store it to buffer, take the root mean
/// square.
template void
OutputStreamsCudaKernels::sampleCuboid<BaseOutputStream::ReduceOperator::kRms>(
    float* samplingBuffer, const float* sourceData, const dim3 topLeftCorner,
    const dim3 bottomRightCorner, const dim3 matrixSize, const size_t nSamples);
/// Sample data inside one cuboid and store it to buffer, take the maximum.
template void
OutputStreamsCudaKernels::sampleCuboid<BaseOutputStream::ReduceOperator::kMax>(
    float* samplingBuffer, const float* sourceData, const dim3 topLeftCorner,
    const dim3 bottomRightCorner, const dim3 matrixSize, const size_t nSamples);
/// Sample data inside one cuboid and store it to buffer, take the minimum.
template void
OutputStreamsCudaKernels::sampleCuboid<BaseOutputStream::ReduceOperator::kMin>(
    float* samplingBuffer, const float* sourceData, const dim3 topLeftCorner,
    const dim3 bottomRightCorner, const dim3 matrixSize, const size_t nSamples);
...
/**
 * CUDA kernel to sample and aggregate the source matrix on the whole domain and
 * apply a reduce operator.
 *
 ...
 */
template <BaseOutputStream::ReduceOperator reduceOp>
__global__ void cudaSampleAll(float* samplingBuffer, const float* sourceData,
                              const size_t nSamples) {
...
void OutputStreamsCudaKernels::sampleAll(float* samplingBuffer,
                                         const float* sourceData,
                                         const size_t nSamples) {
  cudaSampleAll<reduceOp><<<getSamplerGridSize(), getSamplerBlockSize()>>>(
...
/// Sample and the whole domain and apply a defined operator, take the root mean
/// square.
template void
OutputStreamsCudaKernels::sampleAll<BaseOutputStream::ReduceOperator::kRms>(
    float* samplingBuffer, const float* sourceData, const size_t nSamples);
/// Sample and the whole domain and apply a defined operator, take the maximum.
template void
OutputStreamsCudaKernels::sampleAll<BaseOutputStream::ReduceOperator::kMax>(
    float* samplingBuffer, const float* sourceData, const size_t nSamples);
/// Sample and the whole domain and apply a defined operator, take the minimum.
template void
OutputStreamsCudaKernels::sampleAll<BaseOutputStream::ReduceOperator::kMin>(
    float* samplingBuffer, const float* sourceData, const size_t nSamples);
