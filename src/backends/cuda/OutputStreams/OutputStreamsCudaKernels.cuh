 ...
 *
 * @brief     The header file of cuda kernels used for data sampling (output
 * streams).
 *
...
namespace OutputStreamsCudaKernels {
/**
 * @brief Sample the source matrix using the index sensor mask and store data in
 * buffer.
 *
...
void sampleIndex(float* samplingBuffer, const float* sourceData,
                 const size_t* sensorData, const size_t nSamples);

/**
 * @brief Sample data inside one cuboid and store it to buffer. The operation is
 ...
 */
template <BaseOutputStream::ReduceOperator reduceOp>
void sampleCuboid(float* samplingBuffer, const float* sourceData,
                  const dim3 topLeftCorner, const dim3 bottomRightCorner,
 ...
 */
template <BaseOutputStream::ReduceOperator reduceOp>
void sampleAll(float* samplingBuffer, const float* sourceData,
 ...
 */
void postProcessingRms(float* samplingBuffer, const float scalingCoeff,
                       const size_t nSamples);
}  // namespace OutputStreamsCudaKernels
//----------------------------------------------------------------------------------------------------------------------
