 ...
 *
 * @brief     The implementation file containing the class implementing various
 * and 1D FFTs using the cuFFT interface.
 *
 ...
 */
void CufftComplexMatrix::createR2CFftPlanND(
    const DimensionSizes& inMatrixDims) {
  cufftResult cufftError;
  if (Parameters::getInstance().isSimulation3D()) {
    cufftError = cufftPlan3d(&sR2CFftPlanND, static_cast<int>(inMatrixDims.nz),
                             static_cast<int>(inMatrixDims.ny),
                             static_cast<int>(inMatrixDims.nx), CUFFT_R2C);
  } else {
    cufftError = cufftPlan2d(&sR2CFftPlanND, static_cast<int>(inMatrixDims.ny),
                             static_cast<int>(inMatrixDims.nx), CUFFT_R2C);
  }
 ...
 */
void CufftComplexMatrix::createC2RFftPlanND(
    const DimensionSizes& outMatrixDims) {
  cufftResult cufftError;
  if (Parameters::getInstance().isSimulation3D()) {
    cufftError = cufftPlan3d(&sC2RFftPlanND, static_cast<int>(outMatrixDims.nz),
                             static_cast<int>(outMatrixDims.ny),
                             static_cast<int>(outMatrixDims.nx), CUFFT_C2R);
  } else {
    cufftError = cufftPlan2d(&sC2RFftPlanND, static_cast<int>(outMatrixDims.ny),
                             static_cast<int>(outMatrixDims.nx), CUFFT_C2R);
  }
...
/**
 * Create cuFFT plan for 1DX Real-to-Complex transform. Since nz == 1 in the 2D
 * case, there's no need to modify this routine for 2D simulations.
 */
void CufftComplexMatrix::createR2CFftPlan1DX(
    const DimensionSizes& inMatrixDims) {
  // Set dimensions
  ...
  cufftResult_t cufftError =
      cufftPlanMany(&sR2CFftPlan1DX, rank, n, inembed, istride, idist, onembed,
                    ostride, odist, CUFFT_R2C, batch);
...
/**
 * Create cuFFT plan for 1DY Real-to-Complex transform. Since nz == 1 in the 2D
 * case, there's no need to modify this routine for 2D simulations.
 */
void CufftComplexMatrix::createR2CFftPlan1DY(
    const DimensionSizes& inMatrixDims) {
  // Set dimensions
  const int nx = static_cast<int>(inMatrixDims.nx);
  const int ny = static_cast<int>(inMatrixDims.ny);
  ...
  cufftResult_t cufftError =
      cufftPlanMany(&sR2CFftPlan1DY, rank, n, inembed, istride, idist, onembed,
                    ostride, odist, CUFFT_R2C, batch);
...
/**
 * Create cuFFT plan for 1DZ Real-to-Complex transform. This routine throws en
 * exception when called for 2D simulation.
 */
void CufftComplexMatrix::createR2CFftPlan1DZ(
    const DimensionSizes& inMatrixDims) {
  if (Parameters::getInstance().isSimulation2D()) {
    ...
    cufftResult_t cufftError =
        cufftPlanMany(&sR2CFftPlan1DZ, rank, n, inembed, istride, idist,
...
/**
 * Create cuFFT plan for 1DX Complex-to-Real transform. Since nz == 1 in the 2D
 * case, there's no need to modify this routine for 2D simulations.
 */
void CufftComplexMatrix::createC2RFftPlan1DX(
    const DimensionSizes& outMatrixDims) {
  // Set dimensions
  ...
  cufftResult_t cufftError =
      cufftPlanMany(&sC2RFftPlan1DX, rank, n, inembed, istride, idist, onembed,
                    ostride, odist, CUFFT_C2R, batch);
...
/**
 * Create cuFFT plan for 1DY Complex-to-Real transform. Since nz == 1 in the 2D
 * case, there's no need to modify this routine for 2D simulations.
 */
void CufftComplexMatrix::createC2RFftPlan1DY(
    const DimensionSizes& outMatrixDims) {
  // Set dimensions
  const int nx = static_cast<int>(outMatrixDims.nx);
  const int ny = static_cast<int>(outMatrixDims.ny);
  ...
  cufftResult_t cufftError =
      cufftPlanMany(&sC2RFftPlan1DY, rank, n, inembed, istride, idist, onembed,
                    ostride, odist, CUFFT_C2R, batch);
...
/**
 * Create cuFFT plan for 1DZ Complex-to-Real transform. This routine throws en
 * exception when called for 2D simulation.
 */
void CufftComplexMatrix::createC2RFftPlan1DZ(
    const DimensionSizes& outMatrixDims) {
  if (Parameters::getInstance().isSimulation2D()) {
    // Throw error when this routine is called for 2D simulations
    throw std::runtime_error(kErrFmtCannotCallR2CFftPlan1DZfor2D);
    ...
    // The output matrix is transposed with every row padded by a single
    // element.
    int onembed[] = {2 * nzR};
    int ostride = 1;
    int odist = 2 * nzR;
    ...
    cufftResult_t cufftError =
        cufftPlanMany(&sC2RFftPlan1DZ, rank, n, inembed, istride, idist,
...
void CufftComplexMatrix::computeR2CFftND(RealMatrix& inMatrix) {
  ...
  cufftResult_t cufftError = cufftExecR2C(
      sR2CFftPlanND, static_cast<cufftReal*>(inMatrix.getDeviceData()),
...
void CufftComplexMatrix::computeC2RFftND(RealMatrix& outMatrix) {
  // Compute forward cuFFT (if the plan does not exist, it also returns error)
  cufftResult_t cufftError =
      cufftExecC2R(sC2RFftPlanND, reinterpret_cast<cufftComplex*>(mDeviceData),
...
void CufftComplexMatrix::computeR2CFft1DX(RealMatrix& inMatrix) {
  // Compute forward cuFFT (if the plan does not exist, it also returns error)
  cufftResult_t cufftError = cufftExecR2C(
      sR2CFftPlan1DX, static_cast<cufftReal*>(inMatrix.getDeviceData()),
      reinterpret_cast<cufftComplex*>(mDeviceData));
...
/**
 * Computer forward out-of-place 1DY Real-to-Complex transform. The matrix is
 * first X<->Y transposed followed by the 1D FFT. The matrix is left in the
 * transposed format. \n
 *
 * Since nz == 1 in the 2D case, there's no need to modify this routine for 2D
 * simulations.
 */
void CufftComplexMatrix::computeR2CFft1DY(RealMatrix& inMatrix) {
  ...
  TransposeCudaKernels::trasposeReal3DMatrixXY<
      TransposeCudaKernels::TransposePadding::kOutput>(
      mDeviceData, inMatrix.getDeviceData(), dimSizes);

  // Compute forward cuFFT (if the plan does not exist, it also returns error).
  // The FFT is calculated in-place (may be a bit slower than out-of-place,
  // however it does not request additional transfers and memory).
  cufftResult_t cufftError =
      cufftExecR2C(sR2CFftPlan1DY, static_cast<cufftReal*>(mDeviceData),
                   reinterpret_cast<cufftComplex*>(mDeviceData));
...
/**
 * Computer forward out-of-place 1DZ Real-to-Complex transform. This routine
 * throws en exception when called for 2D simulation.
 */
void CufftComplexMatrix::computeR2CFft1DZ(RealMatrix& inMatrix) {
  if (Parameters::getInstance().isSimulation3D()) {
    ...
    TransposeCudaKernels::trasposeReal3DMatrixXZ<
        TransposeCudaKernels::TransposePadding::kOutput>(
        mDeviceData, inMatrix.getDeviceData(), dimSizes);

    // Compute forward cuFFT (if the plan does not exist, it also returns
    // error). The FFT is calculated in-place (may be a bit slower than
    // out-of-place, however it does not request additional transfers and
    // memory).
    cufftResult_t cufftError =
        cufftExecR2C(sR2CFftPlan1DZ, static_cast<cufftReal*>(mDeviceData),
                     reinterpret_cast<cufftComplex*>(mDeviceData));
...
void CufftComplexMatrix::computeC2RFft1DX(RealMatrix& outMatrix) {
  ...
  cufftResult_t cufftError =
      cufftExecC2R(sC2RFftPlan1DX, reinterpret_cast<cufftComplex*>(mDeviceData),
                   static_cast<cufftReal*>(outMatrix.getDeviceData()));
 ...
 * The matrix is taken in the transposed format and transposed at the end into a
 * natural form. \n
 *
 * Since nz == 1 in the 2D case, there's no need to modify this routine for 2D
 * simulations.
...
void CufftComplexMatrix::computeC2RFft1DY(RealMatrix& outMatrix) {
  ...
  // however it does not request additional transfers and memory).
  cufftResult_t cufftError =
      cufftExecC2R(sC2RFftPlan1DY, reinterpret_cast<cufftComplex*>(mDeviceData),
  ...
  TransposeCudaKernels::trasposeReal3DMatrixXY<
      TransposeCudaKernels::TransposePadding::kInput>(outMatrix.getDeviceData(),
                                                      mDeviceData, dimSizes);
}  // end of computeC2RFft1DY
//----------------------------------------------------------------------------------------------------------------------

/**
 * Computer forward out-of-place 1DY Real-to-Complex transform. This routine
 * throws en exception when called for 2D simulation.
 */
void CufftComplexMatrix::computeC2RFft1DZ(RealMatrix& outMatrix) {
  if (Parameters::getInstance().isSimulation3D()) {
    ...
    cufftResult_t cufftError = cufftExecC2R(
        sC2RFftPlan1DZ, reinterpret_cast<cufftComplex*>(mDeviceData),
        static_cast<cufftReal*>(mDeviceData));
    ...
    TransposeCudaKernels::trasposeReal3DMatrixXZ<
        TransposeCudaKernels::TransposePadding::kInput>(
 ...
 */
void CufftComplexMatrix::throwCufftException(
    const cufftResult cufftError, const std::string& transformTypeName) {
  std::string errMsg;
  if (sCufftErrorMessages.find(cufftError) != sCufftErrorMessages.end()) {
    errMsg = Logger::formatMessage(sCufftErrorMessages[cufftError],
                                   transformTypeName.c_str());
  } else  // Unknown error
  {
    errMsg = Logger::formatMessage(kErrFmtCufftUnknownError,
                                   transformTypeName.c_str());
  }
