 ...
 *
 * @copyright Copyright (C) 2019 - 2020 SC\@FIT Research Group, Brno University
 ...
 * Every CUDA block is composed of a 2D mesh of threads. The y dim gives the
 * number of tiles processed simultaneously. Each tile is processed by a single
 * thread warp. The shared memory is used to coalesce memory accesses and the
 * shared memory padding is to eliminate bank conflicts. As a part of the
 * transposition, the matrices can be padded to conform with cuFFT.
 *
 ...
 * @tparam      tilesAtOnce  - How many tiles to transpose at once (4 is
 * default).
 *
 ...
 *
 */
template <TransposeCudaKernels::TransposePadding padding, int warpSize,
          int tilesAtOnce>
__global__ void cudaTrasnposeReal3DMatrixXY(float* outputMatrix,
                                            const float* inputMatrix,
                                            const dim3 dimSizes) {
  // We transpose tilesAtOnce tiles of warp size^2 at the same time, +1 solves
  // bank conflicts.
  ...
  const int nxPadded =
      ((padding == TP::kInput) || (padding == TP::kInputOutput))
          ? 2 * (dimSizes.x / 2 + 1)
          : dimSizes.x;
  const int nyPadded =
      ((padding == TP::kOutput) || (padding == TP::kInputOutput))
          ? 2 * (dimSizes.y / 2 + 1)
          : dimSizes.y;

  const dim3 tileCount = {dimSizes.x / warpSize + 1, dimSizes.y / warpSize + 1,
                          1};
  ...
  for (auto slabIdx = blockIdx.x; slabIdx < dimSizes.z; slabIdx += gridDim.x) {
    ...
    auto copyToShared = [=](int y, int x, int row) {
      ...
      if (globalX < dimSizes.x) {
        sharedTile[threadIdx.y][row][threadIdx.x] =
            inputSlab[globalY * nxPadded + globalX];
      }
    };
    ...
    auto copyToOutput = [=](int y, int x, int row) {
      ...
      if (globalX < dimSizes.y) {
        outputSlab[globalY * nyPadded + globalX] =
    ...
    // Go over the tiles in the y dimension. Multiple tiles processed
    // simultaneously are under each other.
    for (tileIdx.y = threadIdx.y; tileIdx.y < tileCount.y;
         tileIdx.y += blockDim.y) {
      // Go over the tiles in the x dimension.
      for (tileIdx.x = 0; tileIdx.x < tileCount.x; tileIdx.x++) {
        // Go over the rows in a tile and load data. Process only rows lying
        // within the matrix.
        for (auto row = 0; isRowWithinMatrix(tileIdx.y, row, dimSizes.y);
             row++) {
          copyToShared(tileIdx.y, tileIdx.x, row);
        }  // Load data
        __syncwarp();

        // Go over the rows in a transposed tile and store data. Process only
        // rows lying within the matrix.
        for (auto row = 0; isRowWithinMatrix(tileIdx.x, row, dimSizes.x);
             row++) {
...
void TransposeCudaKernels::trasposeReal3DMatrixXY(float* outputMatrix,
                                                  const float* inputMatrix,
                                                  const dim3& dimSizes) {
  // Fixed size at the moment, may be tuned based on the domain shape in the
  // future warpSize set to 32, and 4 tiles processed at once
...
template void TransposeCudaKernels::trasposeReal3DMatrixXY<
    TransposeCudaKernels::TransposePadding::kInput>(float* outputMatrix,
                                                    const float* inputMatrix,
                                                    const dim3& dimSizes);

/// Transpose a real 3D matrix in the X-Y direction, input matrix compact,
/// output matrix padded.
template void TransposeCudaKernels::trasposeReal3DMatrixXY<
    TransposeCudaKernels::TransposePadding::kOutput>(float* outputMatrix,
                                                     const float* inputMatrix,
                                                     const dim3& dimSizes);

/// Transpose a real 3D matrix in the X-Y direction, input and output matrix
/// compact.
template void TransposeCudaKernels::trasposeReal3DMatrixXY<
    TransposeCudaKernels::TransposePadding::kNone>(float* outputMatrix,
                                                   const float* inputMatrix,
                                                   const dim3& dimSizes);
/// Transpose a real 3D matrix in the X-Y direction, input and output matrix
/// padded.
template void TransposeCudaKernels::trasposeReal3DMatrixXY<
    TransposeCudaKernels::TransposePadding::kInputOutput>(
    float* outputMatrix, const float* inputMatrix, const dim3& dimSizes);
//----------------------------------------------------------------------------------------------------------------------
...
__global__ void cudaTrasnposeReal3DMatrixXZ(float* outputMatrix,
  ...
  const dim3 tileCount = {dimSizes.x / warpSize + 1, dimSizes.z / warpSize + 1,
                          1};
  ...
  for (auto row = blockIdx.x; row < dimSizes.y; row += gridDim.x) {
    ...
    auto copyToShared = [=](int y, int x, int slab) {
      ...
      if (globalX < dimSizes.x) {
        sharedTile[threadIdx.y][slab][threadIdx.x] =
            inputMatrix[globalY * nxPadded * dimSizes.y + row * nxPadded +
                        globalX];
      }
    };
    ...
    auto copyToOutput = [=](int y, int x, int slab) {
      ...
      if (globalX < dimSizes.z) {
        outputMatrix[globalY * dimSizes.y * nzPadded + (row * nzPadded) +
                     globalX] = sharedTile[threadIdx.y][threadIdx.x][slab];
      }
    };
    ...
    // Go over all all tiles in the XZ slab. Transpose multiple slabs at the
    // same time (one per Z)
    for (tileIdx.y = threadIdx.y; tileIdx.y < tileCount.y;
         tileIdx.y += blockDim.y) {
      // Go over the tiles in the x dimension.
      for (tileIdx.x = 0; tileIdx.x < tileCount.x; tileIdx.x++) {
        // Go over the slabs in a tile and load data. Process only slabs lying
        // within the matrix.
        for (auto slab = 0; isSlabWithinMatrix(tileIdx.y, slab, dimSizes.z);
             slab++) {
          copyToShared(tileIdx.y, tileIdx.x, slab);
        }  // Load data
        __syncwarp();

        // Go over the slabs in a transposed tile and store data. Process only
        // slabs lying within the matrix.
        for (auto slab = 0; isSlabWithinMatrix(tileIdx.x, slab, dimSizes.x);
             slab++) {
...
//-------------------------------------//
/// Transpose a real 3D matrix in the X-Z direction, input matrix padded, output
/// matrix compact.
template void TransposeCudaKernels::trasposeReal3DMatrixXZ<
    TransposeCudaKernels::TransposePadding::kInput>(float* outputMatrix,
                                                    const float* inputMatrix,
                                                    const dim3& dimSizes);

/// Transpose a real 3D matrix in the X-Z direction, input matrix compact,
/// output matrix padded.
template void TransposeCudaKernels::trasposeReal3DMatrixXZ<
    TransposeCudaKernels::TransposePadding::kOutput>(float* outputMatrix,
                                                     const float* inputMatrix,
...
/// Transpose a real 3D matrix in the X-Z direction, input and output matrix
/// compact.
template void TransposeCudaKernels::trasposeReal3DMatrixXZ<
    TransposeCudaKernels::TransposePadding::kNone>(float* outputMatrix,
                                                   const float* inputMatrix,
