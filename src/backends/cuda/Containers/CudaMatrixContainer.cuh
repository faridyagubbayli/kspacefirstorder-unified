 ...
 *
 * @brief     The header file defining the cuda matrix container used in cuda
 * kernels.
 *
 ...
 * @brief   This container holds pointers to device matrices used in CUDA
 * kernels.
 *
 * @details Since STL containers are not allowed in CUDA, the raw pointers are
 * copied into an array of a predefined size. This array can hold all possible
 * matrices. If some of them is not present in the simulation, the pointer is
 * set to nullptr.
 *
 *          The container is initialized on the host side after the
 * MatrixContainer has been allocated and the pointers to device data
...
class CudaMatrixContainer {
 public:
  ...
  /**
   * @brief Copy matrix records (raw data pointers) inside this container on the
   * host size.
   ...
   * @return Pointer to device data.
   */
  __device__ cuFloatComplex* getComplexData(
      MatrixContainer::MatrixIdx matrixIdx) {
 ...
 private:
  ...
  /// Container holding all device pointers accessed by a MatrixIdx converted to
  /// unsigned int64.
