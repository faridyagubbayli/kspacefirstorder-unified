 ...
 *
 * @brief     The implementation file of the cuda matrix container used in cuda
 * kernels.
 *
 ...
 */
__constant__ CudaMatrixContainer<MatrixContainer::getMatrixIdxCount()>
    cudaMatrixContainer;
...
/**
 * Copy matrix records (raw data pointers) inside this container on the host
 * size.
 */
template <size_t size>
__host__ void CudaMatrixContainer<size>::copyMatrixRecord(
    const MatrixContainer::MatrixIdx matrixIdx,
  ...
  switch (matrixRecord.matrixType) {
    ...
    case MT::kComplex: {
      mMatrixContainer[int(matrixIdx)].complexData =
          static_cast<ComplexMatrix*>(matrixRecord.matrixPtr)
              ->getComplexDeviceData();
      break;
    }

    case MT::kCufft: {
      mMatrixContainer[int(matrixIdx)].complexData =
          static_cast<CufftComplexMatrix*>(matrixRecord.matrixPtr)
              ->getComplexDeviceData();
      break;
    }
    ...
    default: {
      throw std::invalid_argument(Logger::formatMessage(
          kErrFmtBadMatrixType, matrixRecord.matrixName.c_str()));
    }
  }
...
__host__ void CudaMatrixContainer<size>::copyToDevice() {
  cudaCheckErrors(cudaMemcpyToSymbol(cudaMatrixContainer, this,
                                     sizeof(cudaMatrixContainer)));
}  // end of copyToDevice
//----------------------------------------------------------------------------------------------------------------------
