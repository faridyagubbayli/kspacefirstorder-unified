 ...
 *
 * @brief     The header file for all cuda kernels used in the GPU
 ...
 * @details Static class with all cuda kernels used in the solver. This class
 * cannot be instantiated. The on reason why a namespace is not used it the
 * template capabilities of the class that allows to remove deep decision trees.
 *
...
template <Parameters::SimulationDimension simulationDimension =
              Parameters::SimulationDimension::k2D,
          bool rho0ScalarFlag = true, bool bOnAScalarFlag = true,
          bool c0ScalarFlag = true, bool alphaCoefScalarFlag = true>
class SolverCudaKernels {
 public:
   ...
   * @details It is done by calling a kernel that reads a variable set by nvcc
   * compiler.
   *
   ...
   * bsxfun(@times, pml_y_sgy, uy_sgy) - dt .* rho0_sgy_inv .* real(ifftY)
  ...
  //--------------------------------------------//
  /**
   * @brief Compute spatial part of the velocity gradient in between FFTs on
   * uniform grid.
   *
   * <b> Matlab code: </b> \code
  ...
  /**
   * @brief Compute three temporary sums in the new pressure formula in
   ...
   */
  static void computePressureTermsNonlinearPowerLaw(
   ...
   * \endcode
   */
  static void sumPressureTermsNonlinearPowerLaw(
      const RealMatrix& nonlinearTerm, const RealMatrix& absorbTauTerm,
  ...
  /**
   * @brief Compute two temporary sums in the new pressure formula for linear
   * power law case.
   *
   * @param [out] densitySum          - rhox_sgx + rhoy_sgy + rhoz_sgz;
   * @param [out] velocityGradientSum - rho0 * (duxdx + duydy + duzdz);
   */
  static void computePressureTermsLinearPowerLaw(
   ...
   *
   * @param [out] scaledSource - Temporary matrix to insert the source into
   * before scaling.
  ...
  /**
   * @brief Add initial pressure source to the pressure matrix and update
   * density matrices.
   *
   * <b>Matlab code:</b> \code
  ...
  /**
   * @brief Compute acoustic velocity for initial pressure problem, uniform
   * grid.
   * @tparam simulationDimension - Dimensionality of the simulation.
   *
   * <b> Matlab code: </b> \code
