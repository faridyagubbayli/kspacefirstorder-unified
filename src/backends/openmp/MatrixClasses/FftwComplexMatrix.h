 ...
 *
 * @brief     The header file containing the class that implements various FFT
 ...
 * transforms using the FFTW interface.
 * @details Class implementing a single ND (3D, 2D) and many 1D Real-To-Complex
 * and Complex-To-Real transforms using the FFTW interface.
...
class FftwComplexMatrix : public ComplexMatrix {
 public:
   ...
   * @param [in] inMatrix      - Input matrix serving as scratch place for
   * planning.
   * @throw std::runtime_error - If the plan can't be created.
   *
   * @warning Unless FFTW_ESTIMATE flag is specified, the content of the
   * inMatrix is destroyed!
   */
  void createR2CFftPlanND(RealMatrix& inMatrix);
  /**
   * @brief Create FFTW plan for ND (2D/3D) Complex-to-Real transform.
   * @param [in] outMatrix     - Output matrix serving as scratch place for
   * planning.
   * @throw std::runtime_error - If the plan can't be created.
   *
   * @warning Unless FFTW_ESTIMATE flag is specified, the content of the
   * outMatrix is destroyed!
   */
  ...
  //----------------------------------------------//
  /**
   * @brief Create an FFTW plan for 1D Real-to-Complex transform in the x
   * dimension.
   *
   * There are two versions of this routine for GCC + FFTW and ICPC + MKL,
   * otherwise it will not build! \n The FFTW version processes the whole matrix
   * at one while the MKL slab by slab.
   *
   * This routine can be used for both 2D and 3D simulations.
   *
   * @param   [in,out] inMatrix  - Input matrix serving as scratch place for
   * planning.
   * @throw   std::runtime_error - If the plan can't be created.
   *
   * @warning Unless FFTW_ESTIMATE flag is specified, the content of the
  ...
  /**
   * @brief Create an FFTW plan for 1D Real-to-Complex transform in the y
   * dimension.
   *
   * There are two versions of this routine for GCC + FFTW and ICPC + MKL,
   * otherwise it will not build! \n The FFTW version processes the whole matrix
   ...
   * @warning Unless FFTW_ESTIMATE flag is specified, the content of the
   * inMatrix is destroyed!
   * @warning The FFTW matrix must be able to store 2 * (nx * (ny /2 + 1) * nz)
   * elements - possibly more than reduced dims!
   */
  void createR2CFftPlan1DY(RealMatrix& inMatrix);
  /**
   * @brief Create an FFTW plan for 1D Real-to-Complex transform in the z
   * dimension.
   *
   * There are two versions of this routine for GCC + FFTW and ICPC + MKL,
   * otherwise it will not build! \n The FFTW version processes the whole matrix
   ...
   * @warning Unless FFTW_ESTIMATE flag is specified, the content of the
   * inMatrix is destroyed!
   * @warning The FFTW matrix must be able to store 2 * (nx * by * (nz / 2 + 1))
   * elements - possibly more than reduced dims!
   */
   ...
   * This routine can be used for both 2D and 3D simulations.
   *
   * @param   [in,out] outMatrix - Output matrix serving as scratch place for
   * planning.
   * @throw   std::runtime_error - If the plan can't be created.
   *
   * @warning Unless FFTW_ESTIMATE flag is specified, the content of the
  ...
  //----------------------------------------------//
  /**
   * @brief Compute 1D out-of-place Real-to-Complex transform in the x
   * dimension.
   *
   * There are two versions of this routine for GCC+FFTW and ICPC + MKL,
   * otherwise it will not build! The FFTW version processes the whole matrix at
   * one while the MKL slab by slab.
   *
   * This routine can be used for both 2D and 3D simulations.
   *
  ...
  /**
   * @brief Compute 1D out-of-place Complex-to-Real transform in the x
   * dimension.
   *
   * There are two versions of this routine for GCC+FFTW and ICPC + MKL,
   * otherwise it will not build! The FFTW version processes the whole matrix at
  ...
  /**
   * @brief Compute 1D out-of-place Complex-to-Real transform in the y
   * dimension.
   *
   * There are two versions of this routine for GCC+FFTW and ICPC + MKL,
   * otherwise it will not build! The FFTW version processes the whole matrix at
  ...
  /**
   * @brief Compute 1D out-of-place Complex-to-Real transform in the z
   * dimension.
   *
   * There are two versions of this routine for GCC+FFTW and ICPC + MKL,
   * otherwise it will not build! The FFTW version processes the whole matrix at
