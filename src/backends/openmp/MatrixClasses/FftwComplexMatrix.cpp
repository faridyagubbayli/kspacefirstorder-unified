 ...
 * various FFT using the FFTW interface.
 *
...
FftwComplexMatrix::FftwComplexMatrix(const DimensionSizes& dimensionSizes)
    : ComplexMatrix(dimensionSizes),
      ...
      mC2RFftPlan1DZ(nullptr) {}  // end of FftwComplexMatrix
//----------------------------------------------------------------------------------------------------------------------
...
void FftwComplexMatrix::createR2CFftPlanND(RealMatrix& inMatrix) {
  if (Parameters::getInstance().isSimulation3D()) {
    mR2CFftPlanND = fftwf_plan_dft_r2c_3d(
        inMatrix.getDimensionSizes().nz, inMatrix.getDimensionSizes().ny,
        inMatrix.getDimensionSizes().nx, inMatrix.getData(),
        reinterpret_cast<fftwf_complex*>(mData), kFftMeasureFlag);
  } else if (Parameters::getInstance().isSimulation2D()) {
    mR2CFftPlanND = fftwf_plan_dft_r2c_2d(
        inMatrix.getDimensionSizes().ny, inMatrix.getDimensionSizes().nx,
        inMatrix.getData(), reinterpret_cast<fftwf_complex*>(mData),
        kFftMeasureFlag);
  }
...
void FftwComplexMatrix::createC2RFftPlanND(RealMatrix& outMatrix) {
  if (Parameters::getInstance().isSimulation3D()) {
    mC2RFftPlanND = fftwf_plan_dft_c2r_3d(
        outMatrix.getDimensionSizes().nz, outMatrix.getDimensionSizes().ny,
        outMatrix.getDimensionSizes().nx,
        reinterpret_cast<fftwf_complex*>(mData), outMatrix.getData(),
        kFftMeasureFlag);
  } else if (Parameters::getInstance().isSimulation2D()) {
    mC2RFftPlanND = fftwf_plan_dft_c2r_2d(
        outMatrix.getDimensionSizes().ny, outMatrix.getDimensionSizes().nx,
        reinterpret_cast<fftwf_complex*>(mData), outMatrix.getData(),
        kFftMeasureFlag);
  }
...
void FftwComplexMatrix::createR2CFftPlan1DX(RealMatrix& inMatrix) {
  // The FFTW uses here 32b interface although it is internally 64b, it doesn't
  // mind since the size of 1 domain will never be bigger than 2^31 - however it
  // it not a clear solution :)
...
// Intel Compiler + MKL does it slab by slab
  ...
  mR2CFftPlan1DX = fftwf_plan_guru_dft_r2c(
      rank,                                     // 1D FFT rank
...
// GNU Compiler + FFTW does it all at once
  ...
  mR2CFftPlan1DY = fftwf_plan_guru_dft_r2c(
      rank,                                     // 1D FFT rank
...
void FftwComplexMatrix::createR2CFftPlan1DZ(RealMatrix& inMatrix) {
  if (Parameters::getInstance().isSimulation3D()) {
    // The FFTW uses here 32b interface although it is internally 64b, it
    // doesn't mind since the size of 1 domain will never be bigger than 2^31 -
    // however it it not a clear solution :)
    const int nx = static_cast<int>(inMatrix.getDimensionSizes().nx);
    const int ny = static_cast<int>(inMatrix.getDimensionSizes().ny);
    const int nz = static_cast<int>(inMatrix.getDimensionSizes().nz);
...
// Intel Compiler + MKL does it slab by slab
    ...
    mR2CFftPlan1DZ = fftwf_plan_guru_dft_r2c(
        rank,                                     // 1D FFT rank
...
// Intel Compiler + MKL does it slab by slab
  ...
  mC2RFftPlan1DX = fftwf_plan_guru_dft_c2r(
      rank,                                     // 1D FFT rank
      dims,                                     // 1D FFT dimensions of x
      howManyRank,                              // how many in y and z
      howManyDims,                              // Dims and strides in y and z
...
// Intel Compiler + MKL does it slab by slab
  ...
  mC2RFftPlan1DY = fftwf_plan_guru_dft_c2r(
      rank,                                     // 1D FFT rank
      dims,                                     // 1D FFT dimensions of y
      howManyRank,                              // How many in x and z
      howManyDims,                              // Dims and strides in x and z
...
// Intel Compiler + MKL does it slab by slab
    ...
    mC2RFftPlan1DZ = fftwf_plan_guru_dft_c2r(
        rank,                                     // 1D FFT rank
        dims,                                     // 1D FFT dimensions of z
        howManyRank,                              // how many in x and y
        howManyDims,                              // Dims and strides in x and y
...
void FftwComplexMatrix::computeR2CFftND(RealMatrix& inMatrix) {
  if (mR2CFftPlanND) {
    fftwf_execute_dft_r2c(mR2CFftPlanND, inMatrix.getData(),
...
void FftwComplexMatrix::computeC2RFftND(RealMatrix& outMatrix) {
  if (mC2RFftPlanND) {
    fftwf_execute_dft_c2r(mC2RFftPlanND,
                          reinterpret_cast<fftwf_complex*>(mData),
                          outMatrix.getData());
  } else {
...
void FftwComplexMatrix::computeR2CFft1DX(RealMatrix& inMatrix) {
  // This will work for both 2D and 3D simulations because of Nz = 1 for 2D
  // cases.
  if (mR2CFftPlan1DX) {
// GNU Compiler + FFTW
#if (defined(__GNUC__) || defined(__GNUG__)) && \
    !(defined(__clang__) || defined(__INTEL_COMPILER))
    fftwf_execute_dft_r2c(mR2CFftPlan1DX, inMatrix.getData(),
                          reinterpret_cast<fftwf_complex*>(mData));
#endif

// Intel Compiler + MKL
#if (defined(__INTEL_COMPILER))
    const DimensionSizes dims =
        Parameters::getInstance().getFullDimensionSizes();
    for (size_t slab_id = 0; slab_id < dims.nz; slab_id++) {
      fftwf_execute_dft_r2c(
          mR2CFftPlan1DX, &inMatrix.getData()[slab_id * dims.nx * dims.ny],
...
#if (defined(__GNUC__) || defined(__GNUG__)) && \
    !(defined(__clang__) || defined(__INTEL_COMPILER))
    fftwf_execute_dft_r2c(mR2CFftPlan1DY, inMatrix.getData(),
                          reinterpret_cast<fftwf_complex*>(mData));
#endif
...
// Intel Compiler + MKL
    ...
    for (size_t slab_id = 0; slab_id < dims.nz; slab_id++) {
      fftwf_execute_dft_r2c(
          mR2CFftPlan1DY, &inMatrix.getData()[slab_id * dims.nx * dims.ny],
...
void FftwComplexMatrix::computeR2CFft1DZ(RealMatrix& inMatrix) {
  // This throws an error if called for 2D  simulations since the plan is not
  // created.
  if (mR2CFftPlan1DZ) {
// GNU Compiler + FFTW
#if (defined(__GNUC__) || defined(__GNUG__)) && \
    !(defined(__clang__) || defined(__INTEL_COMPILER))
    fftwf_execute_dft_r2c(mR2CFftPlan1DZ, inMatrix.getData(),
                          reinterpret_cast<fftwf_complex*>(mData));
#endif
