...
FftwRealMatrix::FftwRealMatrix(const DimensionSizes& dimensionSizes)
    : RealMatrix(dimensionSizes),
      mInPlaceR2RPlans1DY(),
      mOutPlaceR2RPlans1DY() {}  // end of FftwRealMatrix
//----------------------------------------------------------------------------------------------------------------------
...
// Intel Compiler + MKL
  ...
  const std::array<TransformKind, 4> usedTransforms{
      TransformKind::kDct2, TransformKind::kDct3, TransformKind::kDct4,
      TransformKind::kDst4};

  for (const auto kind : usedTransforms) {
    fftwf_r2r_kind fftwKind = static_cast<fftwf_r2r_kind>(kind);

    mInPlaceR2RPlans1DY[kind] =
        fftwf_plan_guru_r2r(rank,              // 1D FFT rank
    ...
    if (!mInPlaceR2RPlans1DY[kind]) {
      throw std::runtime_error(
          Logger::formatMessage(kErrFmtCreateR2RFftPlan1D, kind));
    }

// MKL version do not use out-of-place r2r transforms
#if (defined(__GNUC__) || defined(__GNUG__)) && \
    !(defined(__clang__) || defined(__INTEL_COMPILER))
    mOutPlaceR2RPlans1DY[kind] =
        fftwf_plan_guru_r2r(rank,                // 1D FFT rank
                            dims,                // 1D FFT dimensions of y
                            howManyRank,         // How many in x and z
                            howManyDims,         // Dims and strides in x and z
...
// Intel compiler + MKL
  ...
  if (mInPlaceR2RPlans1DY[kind]) {
    // Transpose matrix
    mkl_somatcopy('r', 't', mDimensionSizes.ny, mDimensionSizes.nx, 1.0f,
                  inMatrix.getData(), mDimensionSizes.nx, mData,
                  mDimensionSizes.ny);
...
#pragma omp parallel for schedule(static)
    ...
    throw std::runtime_error(
        Logger::formatMessage(kErrFmtExecuteR2RFftPlan1D, int(kind)));
  }
...
// Intel compiler + MKL
  ...
  if (mInPlaceR2RPlans1DY[kind]) {
    // Transpose matrix
    mkl_simatcopy('r', 't', mDimensionSizes.ny, mDimensionSizes.nx, 1.0f, mData,
                  mDimensionSizes.nx, mDimensionSizes.ny);
