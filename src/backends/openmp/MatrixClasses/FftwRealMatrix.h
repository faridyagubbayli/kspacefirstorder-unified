 ...
 *
 * @brief     The header file containing the class that implements various DTT
 ...
 * @details Class implementing 1D Real-To-Real transforms along the X dimension
 * on the 2D data.
 *
 */
class FftwRealMatrix : public RealMatrix {
 public:
  /**
   * @enum    TransformKind
   * @brief   Different kinds of Real-to-Real transforms as defined in FFTW
   * library.
   * @details All used kinds of Real-to-Real transforms in this code. The enum
   * relies on the FFTW kind ordering in interval [3 - 10].
   */
  ...
  /**
   * @brief Create FFTW plans for 1D Real-to-Real transforms over the Y
   * dimension used for WSWA symmetry.
  ...
  /**
   * @brief Compute forward out-of-place 1D R2R transforms of a given kind on 2D
   * domain over the Y dimension.
   * @param [in] kind     - Kind of the transform.
   * @param [in] inMatrix - Input matrix.
   *
   * @throw std::runtime_error - If the plan is not valid.
   */
  void computeForwardR2RFft1DY(const TransformKind kind, RealMatrix& inMatrix);

  /**
   * @brief Compute inverse out-of-place 1D R2R transforms of a given kind on 2D
   * domain over the Y dimension.
   * @param [in]  kind      - Kind of the transform.
   ...
   * @throw std::runtime_error - If the plan is not valid.
   */
  void computeInverseR2RFft1DY(const TransformKind kind, RealMatrix& outMatrix);

  /**
   * @brief Compute in-place 1D R2R transforms of a given kind on 2D domain over
   * the Y dimension.
   * @param [in] kind - Kind of the transform.
   *
   * @throw std::runtime_error - If the plan is not valid.
 ...
 protected:
  ...
  /// Map with FFTW plans for the 1D Real-to-Real transforms on 2D matrix,
  /// in-palace.
