 ...
 */
std::map<Parameters::ParameterNameIdx, MatrixName>
    Parameters::sParameterHdf5Names{
        ...
        {Parameters::ParameterNameIdx::kNonUniformGridFlag,
        ...
        {Parameters::ParameterNameIdx::kInitialPressureSourceFlag,
         "p0_source_flag"},
        {Parameters::ParameterNameIdx::kTransducerSourceFlag,
        ...
        {Parameters::ParameterNameIdx::kTransducerSourceInput,
        ...
        {Parameters::ParameterNameIdx::kSensorMaskCorners,
         "sensor_mask_corners"},
...
/**
 * Parse command line and read scalar values from the input file to initialize
 ...
 * For 2D simulations, all flags not found in the input file left at default
 * values.
 */
void Parameters::printSimulatoinSetup() {
  // In full version, print out file format version
  Logger::log(Logger::LogLevel::kFull, kOutFmtFileVersion,
              getFileHeader().getFileVersionName().c_str());

  Logger::log(Logger::LogLevel::kBasic, kOutFmtNumberOfThreads,
              getNumberOfThreads());
  Logger::log(Logger::LogLevel::kBasic, kOutFmtProcessorNameRight,
...
/**
 * Read scalar values from the input HDF5 file. 2D medium can be normal or
 * axisymmetric but 3D only normal.
 */
void Parameters::readScalarsFromInputFile() {
  ...
  if (!mFileHeader.checkFileVersion()) {
    throw ios::failure(Logger::formatMessage(
        kErrFmtBadFileVersion, mFileHeader.getFileVersionName().c_str(),
        getInputFileName().c_str(),
  ...
  auto readIntValue = [this, rootGroup](ParameterNameIdx paramIdx,
                                        bool present = true) -> size_t {
    size_t value = 0l;
    if (present) {
      mInputFile.readScalarValue(rootGroup, sParameterHdf5Names[paramIdx],
                                 value);
    }
    return value;
  };  // end of readIntValue
  ...
  auto readFloatValue = [this, rootGroup](ParameterNameIdx paramIdx,
                                          bool present = true) -> float {
    float value = 0.0f;
    if (present) {
      mInputFile.readScalarValue(rootGroup, sParameterHdf5Names[paramIdx],
                                 value);
  ...
  auto isScalar = [this, rootGroup,
                   scalarSizes](ParameterNameIdx paramIdx) -> bool {
    return (mInputFile.getDatasetDimensionSizes(
                rootGroup, sParameterHdf5Names[paramIdx]) == scalarSizes);
  };  // end of isScalar
  ...
  if (mFileHeader.getFileVersion() == Hdf5FileHeader::FileVersion::kVersion12) {
    mAxisymmetricFlag = (bool(readIntValue(PI::kAxisymmetricFlag)) &&
                         mFullDimensionSizes.is2D());
  }
  ...
  mRho0SgzScalar =
      (mRho0ScalarFlag) ? readFloatValue(PI::kRho0Sgz, isSimulation3D()) : 0.0f;
  ...
  if (mAbsorbingFlag != AbsorptionType::kLossless) {
    ...
    mAlphaCoeffScalar =
        (mAlphaCoeffScalarFlag) ? readFloatValue(PI::kAlphaCoeff) : 0.0f;
  }

  //---------------------------------------------------- Sensors
  //-----------------------------------------------------//
  // If the file is of version 1.0, there must be a sensor mask index (backward
  // compatibility)
  if (mFileHeader.getFileVersion() == Hdf5FileHeader::FileVersion::kVersion10) {
    mSensorMaskIndexSize = mInputFile.getDatasetSize(
        rootGroup, sParameterHdf5Names[PI::kSensorMaskIndex]);
        ...
        mSensorMaskCornersSize =
            mInputFile
                .getDatasetDimensionSizes(
                    rootGroup, sParameterHdf5Names[PI::kSensorMaskCorners])
                .ny;
        break;
      }
    }  // switch
  ...
  mVelocityZSourceFlag =
      readIntValue(PI::kVelocityZSourceFlag, isSimulation3D());

  // Source sizes.
  mTransducerSourceInputSize =
      (mTransducerSourceFlag)
          ? mInputFile.getDatasetSize(
                rootGroup, sParameterHdf5Names[PI::kTransducerSourceInput])
          : 0l;
  ...
  if ((mTransducerSourceFlag > 0) || (mVelocityXSourceFlag > 0) ||
    ...
    mVelocitySourceIndexSize = mInputFile.getDatasetSize(
        rootGroup, sParameterHdf5Names[PI::kVelocitySourceIndex]);
  }

  // Convert the size_t value to enum
  auto getSourceMode = [](size_t sourceModeNumericValue,
                          const string& errorMessage) -> SourceMode {
  ...
  if ((mVelocityXSourceFlag > 0) || (mVelocityYSourceFlag > 0) ||
    ...
    mVelocitySourceMode = getSourceMode(readIntValue(PI::kVelocitySourceMode),
                                        kErrFmtBadVelocitySourceMode);
  } else {
  ...
  if (mPressureSourceFlag != 0) {
    ...
    // Convert the size_t value to enum
    mPressureSourceMode = getSourceMode(readIntValue(PI::kPressureSourceMode),
                                        kErrFmtBadPressureSourceMode);

    mPressureSourceIndexSize = mInputFile.getDatasetSize(
        rootGroup, sParameterHdf5Names[PI::kPressureSourceIndex]);
  } else {
...
void Parameters::writeScalarsToOutputFile() {
  ...
  // We need two function since C++-11 doesn't allow lambda overloading
  auto writeIntValue = [this, rootGroup](ParameterNameIdx paramIdx,
                                         size_t value, bool present = true) {
    if (present) {
      mOutputFile.writeScalarValue(rootGroup, sParameterHdf5Names[paramIdx],
                                   value);
    }
  };  // end of writeIntValue

  // Lambda to write a scalar the output file for float.
  auto writeFloatValue = [this, rootGroup](ParameterNameIdx paramIdx,
                                           float value, bool present = true) {
    if (present) {
      mOutputFile.writeScalarValue(rootGroup, sParameterHdf5Names[paramIdx],
                                   value);
    }
  ...
  // Write dimension sizes (Z is always written to distinguish 2D and 3D
  // simulations)
  ...
  writeIntValue(PI::kVelocityZSourceFlag, mVelocityZSourceFlag,
                isSimulation3D());
 ...
 * This routine was inspired by this project
...
std::string Parameters::getProcessorName() const {
  ...
  if (sysctlbyname("machdep.cpu.brand_string", buffer, &bufferSize, nullptr,
                   0) == 0) {
  ...
  auto cpuid = [](unsigned int funcId, unsigned int subFuncId,
                  ProcessorRegistry& regs) {
// Linux build
#ifdef __linux__
    asm volatile("cpuid"
...
void Parameters::printMediumProperties() {
  ...
  Logger::log(Logger::LogLevel::kAdvanced, kOutFmtWavePropagation,
              getNonLinearFlag() ? kOutFmtAbsorbtionTypeNonLinear.c_str()
                                 : kOutFmtAbsorbtionTypeLinear.c_str());

  switch (getAbsorbingFlag()) {
    case AbsorptionType::kLossless: {
      Logger::log(Logger::LogLevel::kAdvanced, kOutFmtAbsorbtionType,
                  kOutFmtAbsorbtionLossless.c_str());
      break;
    }
    case AbsorptionType::kPowerLaw: {
      Logger::log(Logger::LogLevel::kAdvanced, kOutFmtAbsorbtionType,
                  kOutFmtAbsorbtionPowerLaw.c_str());
      break;
    }
    case AbsorptionType::kStokes: {
      Logger::log(Logger::LogLevel::kAdvanced, kOutFmtAbsorbtionType,
                  kOutFmtAbsorbtionStokes.c_str());
      break;
    }
  }

  if (getC0ScalarFlag()) {
    Logger::log(Logger::LogLevel::kAdvanced, kOutFmtMediumParameters,
                kOutFmtMediumParametersHomegeneous.c_str());
  } else {
    if ((getNonLinearFlag() && getBOnAScalarFlag()) ||
        ((getAbsorbingFlag() != AbsorptionType::kLossless) &&
         getAlphaCoeffScalarFlag())) {
      Logger::log(Logger::LogLevel::kAdvanced, kOutFmtMediumParameters,
                  kOutFmtMediumParametersHeterogeneousC0andRho0.c_str());
    } else {
      Logger::log(Logger::LogLevel::kAdvanced, kOutFmtMediumParameters,
                  kOutFmtMediumParametersHeterogeneous.c_str());
    }
  }
...
void Parameters::printSourceInfo() {
  ...
  auto printTimeVaryingSourceInfo = [=](size_t sourceFlag, size_t sourceMany,
                                        SourceMode sourceMode, size_t indexSize,
                                        const string& sourceName) {
    Logger::log(Logger::LogLevel::kAdvanced, sourceName);
    // Type
    Logger::log(Logger::LogLevel::kAdvanced, kOutFmtSourceType,
                sourceMany ? kOutFmtSourceTypeMany.c_str()
                           : kOutFmtSourceTypeSingle.c_str());
    // Mode
    Logger::log(Logger::LogLevel::kAdvanced, kOutFmtSourceMode,
                getSourceModeText(sourceMode).c_str());
    // size = sensor mask + signal size
    const float sensorSize =
        maskToMB(indexSize) + ((sourceMany) ? floatToMB(indexSize * sourceFlag)
                                            : floatToMB(sourceFlag));

    Logger::log(Logger::LogLevel::kAdvanced, kOutFmtSourceMemoryUsage.c_str(),
                sensorSize);
  ...
  if (getInitialPressureSourceFlag()) {
    ...
    Logger::log(Logger::LogLevel::kAdvanced, kOutFmtSourceMemoryUsage.c_str(),
                floatToMB(mFullDimensionSizes.nElements()));
  }

  if (getPressureSourceFlag()) {
    printTimeVaryingSourceInfo(getPressureSourceFlag(), getPressureSourceMany(),
  ...
  if (getVelocityXSourceFlag()) {
    printTimeVaryingSourceInfo(getVelocityXSourceFlag(),
                               getVelocitySourceMany(), getVelocitySourceMode(),
  ...
  if (getTransducerSourceFlag()) {
    ...
    float sensorSize = maskToMB(2 * getVelocitySourceIndexSize()) +
                       floatToMB(getTransducerSourceInputSize());
...
void Parameters::printSensorInfo() {
  ...
  auto printSensor = [floatToMB](bool sensorFlag, size_t sensorSize,
                                 const string& sensorName) {
    if (sensorFlag) {
      // Print the sensor name
      Logger::log(Logger::LogLevel::kAdvanced, kOutFmtSensorName,
                  sensorName.c_str());
      // Print sensor file size
      Logger::log(Logger::LogLevel::kAdvanced, kOutFmtSensorFileUsage,
                  floatToMB(sensorSize));
    }
  };  // end of printSensor
  ...
  if (getSensorMaskType() == SensorMaskType::kCorners) {
    ...
    // Here we have to read the cuboid sensor mask. It's not a good solution,
    // but only feasible without code refactoring.
    IndexMatrix sensorMaskCuboid(
        DimensionSizes(6, getSensorMaskCornersSize(), 1));
    sensorMaskCuboid.readData(
        mInputFile, sParameterHdf5Names[ParameterNameIdx::kSensorMaskCorners]);
    aggregatedSensorSize = sensorMaskCuboid.getSizeOfAllCuboids();
  }

  // Print starting time step
  Logger::log(Logger::LogLevel::kAdvanced, kOutFmtSamplingStartsAt,
              getSamplingStartTimeIndex() + 1);
  Logger::log(Logger::LogLevel::kAdvanced, kOutFmtSeparator);

  // Time series
  printSensor(getStorePressureRawFlag(), aggregatedSensorSize * sampledSteps,
  ...
  printSensor(getStorePressureRmsFlag(), aggregatedSensorSize,
              kOutFmtSensorPressureRms.c_str());
  printSensor(getStorePressureMaxFlag(), aggregatedSensorSize,
              kOutFmtSensorPressureMax.c_str());
  printSensor(getStorePressureMinFlag(), aggregatedSensorSize,
              kOutFmtSensorPressureMin.c_str());
  printSensor(getStorePressureMaxAllFlag(), nElements,
              kOutFmtSensorPressureMaxAll.c_str());
  printSensor(getStorePressureMinAllFlag(), nElements,
              kOutFmtSensorPressureMinAll.c_str());
  printSensor(getStorePressureFinalAllFlag(), nElements,
              kOutFmtSensorPressureFinal.c_str());

  // Aggregated velocity quantities
  printSensor(getStoreVelocityRmsFlag(), 3 * aggregatedSensorSize,
              kOutFmtSensorVelocityRms.c_str());
  printSensor(getStoreVelocityMaxFlag(), 3 * aggregatedSensorSize,
              kOutFmtSensorVelocityMax.c_str());
  printSensor(getStoreVelocityMinFlag(), 3 * aggregatedSensorSize,
              kOutFmtSensorVelocityMin.c_str());
  printSensor(getStoreVelocityMaxAllFlag(), 3 * nElements,
              kOutFmtSensorVelocityMaxAll.c_str());
  printSensor(getStoreVelocityMinAllFlag(), 3 * nElements,
              kOutFmtSensorVelocityMinAll.c_str());
  printSensor(getStoreVelocityFinalAllFlag(), 3 * nElements,
              kOutFmtSensorVelocityFinal.c_str());
...
Parameters::Parameters()
    : mCommandLineParameters(),
      ...
      mCheckpointFile(),
      mFileHeader(),
      mFullDimensionSizes(0, 0, 0),
      mReducedDimensionSizes(0, 0, 0),
      mAxisymmetricFlag(false),
      mNt(0),
      mTimeIndex(0),
      mTimeStepsToCheckpoint(std::numeric_limits<size_t>::max()),
      mDt(0.0f),
      mDx(0.0f),
      mDy(0.0f),
      mDz(0.0f),
      ...
      mNonLinearFlag(0),
      mAlphaCoeffScalarFlag(false),
      mAlphaCoeffScalar(0.0f),
      mAlphaPower(0.0f),
      mAbsorbEtaScalar(0.0f),
      mAbsorbTauScalar(0.0f),
      mBOnAScalarFlag(false),
      mBOnAScalar(0.0f),
      mPmlXSize(0),
      mPmlYSize(0),
      mPmlZSize(0),
      ...
      mVelocitySourceIndexSize(0),
      mPressureSourceMode(SourceMode::kDirichlet),
      mPressureSourceMany(0),
      mVelocitySourceMode(SourceMode::kDirichlet),
      mVelocitySourceMany(0),
      mSensorMaskType(SensorMaskType::kIndex),
      mSensorMaskIndexSize(0),
      mSensorMaskCornersSize(0) {}  // end of Parameters
//----------------------------------------------------------------------------------------------------------------------
