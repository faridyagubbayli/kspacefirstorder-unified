 ...
 */
std::map<OutputStreamContainer::OutputStreamIdx, MatrixName>
    OutputStreamContainer::sOutputStreamHdf5Names{
...
OutputStreamContainer::OutputStreamContainer()
    : mContainer() {}  // end of OutputStreamContainer
//----------------------------------------------------------------------------------------------------------------------
...
/**
 * Add all streams in the simulation to the container, set all streams records
 * here!
 */
void OutputStreamContainer::init(const MatrixContainer& matrixContainer) {
  ...
  float* tempBuffX =
      matrixContainer.getMatrix<RealMatrix>(MI::kTemp1RealND).getData();
  float* tempBuffY =
      matrixContainer.getMatrix<RealMatrix>(MI::kTemp2RealND).getData();
  float* tempBuffZ =
      (isSimulation3D)
          ? matrixContainer.getMatrix<RealMatrix>(MI::kTemp3RealND).getData()
          : nullptr;

  //-------------------------------------------------- pressure
  //------------------------------------------------------//
  if (params.getStorePressureRawFlag()) {
    addOutputStream(OI::kPressureRaw, matrixContainer, MI::kP, RO::kNone,
                    kAlways, tempBuffX);
  }
  ...
  if (params.getStorePressureMaxAllFlag()) {
    addWholeDomainOutputStream(OI::kPressureMaxAll, matrixContainer, MI::kP,
                               RO::kMax);
  }

  if (params.getStorePressureMinAllFlag()) {
    addWholeDomainOutputStream(OI::kPressureMinAll, matrixContainer, MI::kP,
                               RO::kMin);
  }
  ...
  if (params.getStoreVelocityRawFlag()) {
    addOutputStream(OI::kVelocityXRaw, matrixContainer, MI::kUxSgx, RO::kNone,
                    kAlways, tempBuffX);
    addOutputStream(OI::kVelocityYRaw, matrixContainer, MI::kUySgy, RO::kNone,
                    kAlways, tempBuffY);
    addOutputStream(OI::kVelocityZRaw, matrixContainer, MI::kUzSgz, RO::kNone,
                    isSimulation3D, tempBuffZ);
  }

  if (params.getStoreVelocityNonStaggeredRawFlag()) {
    addOutputStream(OI::kVelocityXNonStaggeredRaw, matrixContainer,
                    MI::kUxShifted, RO::kNone, kAlways, tempBuffX);
    addOutputStream(OI::kVelocityYNonStaggeredRaw, matrixContainer,
                    MI::kUyShifted, RO::kNone, kAlways, tempBuffY);
    addOutputStream(OI::kVelocityZNonStaggeredRaw, matrixContainer,
                    MI::kUzShifted, RO::kNone, isSimulation3D, tempBuffZ);
  }
  ...
  if (params.getStoreVelocityRmsFlag()) {
    ...
    addOutputStream(OI::kVelocityZRms, matrixContainer, MI::kUzSgz, RO::kRms,
                    isSimulation3D);
  }
  ...
  if (params.getStoreVelocityMaxFlag()) {
    ...
    addOutputStream(OI::kVelocityZMax, matrixContainer, MI::kUzSgz, RO::kMax,
                    isSimulation3D);
  }
  ...
  if (params.getStoreVelocityMinFlag()) {
    ...
    addOutputStream(OI::kVelocityZMin, matrixContainer, MI::kUzSgz, RO::kMin,
                    isSimulation3D);
  }

  if (params.getStoreVelocityMaxAllFlag()) {
    addWholeDomainOutputStream(OI::kVelocityXMaxAll, matrixContainer,
                               MI::kUxSgx, RO::kMax);
    addWholeDomainOutputStream(OI::kVelocityYMaxAll, matrixContainer,
                               MI::kUySgy, RO::kMax);
    addWholeDomainOutputStream(OI::kVelocityZMaxAll, matrixContainer,
                               MI::kUzSgz, RO::kMax, isSimulation3D);
  }

  if (params.getStoreVelocityMinAllFlag()) {
    addWholeDomainOutputStream(OI::kVelocityXMinAll, matrixContainer,
                               MI::kUxSgx, RO::kMin);
    addWholeDomainOutputStream(OI::kVelocityYMinAll, matrixContainer,
                               MI::kUySgy, RO::kMin);
    addWholeDomainOutputStream(OI::kVelocityZMinAll, matrixContainer,
                               MI::kUzSgz, RO::kMin, isSimulation3D);
  }
}  // end of init
//----------------------------------------------------------------------------------------------------------------------
 ...
 */
void OutputStreamContainer::addOutputStream(
    const OutputStreamIdx streamIdx, const MatrixContainer& matrixContainer,
  ...
  if (present) {
    if (params.getSensorMaskType() == Parameters::SensorMaskType::kIndex) {
      mContainer[streamIdx] = new IndexOutputStream(
          params.getOutputFile(), sOutputStreamHdf5Names[streamIdx],
          matrixContainer.getMatrix<RealMatrix>(sampledMatrixIdx),
          matrixContainer.getMatrix<IndexMatrix>(MI::kSensorMaskIndex),
          reduceOp, bufferToReuse);
    } else {
      mContainer[streamIdx] = new CuboidOutputStream(
          params.getOutputFile(), sOutputStreamHdf5Names[streamIdx],
          matrixContainer.getMatrix<RealMatrix>(sampledMatrixIdx),
 ...
 * Add a new output stream into the container.
 */
void OutputStreamContainer::addWholeDomainOutputStream(
    const OutputStreamIdx streamIdx, const MatrixContainer& matrixContainer,
    const MatrixContainer::MatrixIdx sampledMatrixIdx,
    const BaseOutputStream::ReduceOperator reduceOp, const bool present) {
  Parameters& params = Parameters::getInstance();
  ...
  if (present) {
    mContainer[streamIdx] = new WholeDomainOutputStream(
        params.getOutputFile(), sOutputStreamHdf5Names[streamIdx],
        matrixContainer.getMatrix<RealMatrix>(sampledMatrixIdx), reduceOp);
  }
