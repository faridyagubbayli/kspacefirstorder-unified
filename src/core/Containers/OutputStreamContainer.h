...
class OutputStreamContainer {
 public:
  ...
  enum class OutputStreamIdx {
    ...
    /// Pressure in the last time step - not in the container but flushed
    /// directly from the corresponding matrix.
    kFinalPressure,
    /// Velocity in the last time step in x - not in the container but flushed
    /// directly from the corresponding matrix.
    kFinalVelocityX,
    /// Velocity in the last time step in y - not in the container but flushed
    /// directly from the corresponding matrix.
    kFinalVelocityY,
    /// Velocity in the last time step in z - not in the container but flushed
    /// directly from the corresponding matrix.
    kFinalVelocityZ,
  ...
  /**
   * @brief   Add all streams in the simulation to the container based on the
   * simulation parameters.
   * @details Please note, the matrix container has to be populated before
 ...
 protected:
   ...
   * @param [in] present          - Is the stream present?
   */
  void addWholeDomainOutputStream(
 ...
 private:
  /// Container holding the names of all possible streams present in the output
  /// file.
