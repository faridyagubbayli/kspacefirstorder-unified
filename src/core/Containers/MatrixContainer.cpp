...
// CUDA uses RealMatrix directly, no separate FftwRealMatrix
#else
 ...
 */
std::map<MatrixContainer::MatrixIdx, MatrixName>
    MatrixContainer::sMatrixHdf5Names{
        ...
        // This matrix is in special cases used to preload alpha_coeff to
        // calculate absorb tau
 ...
 * Constructor.
 */
MatrixContainer::MatrixContainer() : mContainer() {}  // end of Constructor.
//----------------------------------------------------------------------------------------------------------------------
...
/**
 * This function creates the list of matrices being used in the simulation. It
 * is done based on the simulation parameters, type and the dimensionality. All
 * matrices records are created here.
 */
void MatrixContainer::init() {
  ...
  // Lambda routine to add a new matrix into the container. By default the
  // matrix is always added into the container.
  auto addMatrix = [this](MI matrixIdx, MT matrixType,
                          DimensionSizes dimensionSizes, bool load,
                          bool checkpoint, bool present = true) {
    // Do not allocate the matrix if the simulation is not 3D and the matrix
    // holds third dimension.
    if (present) {
      mContainer[matrixIdx] =
          MatrixRecord(matrixType, dimensionSizes, load, checkpoint,
                       sMatrixHdf5Names[matrixIdx]);
    }
  };  // end of addMatrix
  ...
  if (!params.getRho0ScalarFlag()) {
    ...
    addMatrix(MI::kDtRho0Sgz, MT::kReal, fullDims, kLoad, kNoCheckpoint,
              isSimulation3D);
  }

  // Derivative operators are now generated, if they appear in the input file,
  // they are ignored.
  addMatrix(MI::kDdxKShiftPosR, MT::kComplex,
            DimensionSizes(reducedDims.nx, 1, 1), kNoLoad, kNoCheckpoint);
  addMatrix(MI::kDdxKShiftNegR, MT::kComplex,
            DimensionSizes(reducedDims.nx, 1, 1), kNoLoad, kNoCheckpoint);

  if (isSimulationAS) {  // Axisymmetric coordinates
    addMatrix(MI::kDdyKWswa, MT::kReal, DimensionSizes(1, reducedDims.ny, 1),
              kNoLoad, kNoCheckpoint);
    addMatrix(MI::kDdyKHahs, MT::kReal, DimensionSizes(1, reducedDims.ny, 1),
              kNoLoad, kNoCheckpoint);
    addMatrix(MI::kYVecSg, MT::kReal, DimensionSizes(1, reducedDims.ny, 1),
              kNoLoad, kNoCheckpoint);
  } else {  // Normal coordinates
    addMatrix(MI::kDdyKShiftPos, MT::kComplex,
              DimensionSizes(1, reducedDims.ny, 1), kNoLoad, kNoCheckpoint);
    addMatrix(MI::kDdyKShiftNeg, MT::kComplex,
              DimensionSizes(1, reducedDims.ny, 1), kNoLoad, kNoCheckpoint);

    addMatrix(MI::kDdzKShiftPos, MT::kComplex,
              DimensionSizes(1, 1, reducedDims.nz), kNoLoad, kNoCheckpoint,
              isSimulation3D);
    addMatrix(MI::kDdzKShiftNeg, MT::kComplex,
              DimensionSizes(1, 1, reducedDims.nz), kNoLoad, kNoCheckpoint,
              isSimulation3D);
  }  // k-space variables

  // Pml variables
  addMatrix(MI::kPmlXSgx, MT::kReal, DimensionSizes(fullDims.nx, 1, 1), kNoLoad,
            kNoCheckpoint);
  addMatrix(MI::kPmlYSgy, MT::kReal, DimensionSizes(1, fullDims.ny, 1), kNoLoad,
            kNoCheckpoint);
  addMatrix(MI::kPmlZSgz, MT::kReal, DimensionSizes(1, 1, fullDims.nz), kNoLoad,
            kNoCheckpoint, isSimulation3D);

  addMatrix(MI::kPmlX, MT::kReal, DimensionSizes(fullDims.nx, 1, 1), kNoLoad,
            kNoCheckpoint);
  addMatrix(MI::kPmlY, MT::kReal, DimensionSizes(1, fullDims.ny, 1), kNoLoad,
            kNoCheckpoint);
  addMatrix(MI::kPmlZ, MT::kReal, DimensionSizes(1, 1, fullDims.nz), kNoLoad,
            kNoCheckpoint, isSimulation3D);
  ...
  if (params.getAbsorbingFlag() == Parameters::AbsorptionType::kPowerLaw) {
    ...
    addMatrix(MI::kAbsorbNabla1, MT::kReal, reducedDims, kNoLoad,
              kNoCheckpoint);
    addMatrix(MI::kAbsorbNabla2, MT::kReal, reducedDims, kNoLoad,
              kNoCheckpoint);
  ...
  if (params.getNonUniformGridFlag() != 0) {
    addMatrix(MI::kDxudxn, MT::kReal, DimensionSizes(fullDims.nx, 1, 1), kLoad,
              kNoCheckpoint);
    addMatrix(MI::kDyudyn, MT::kReal, DimensionSizes(1, fullDims.ny, 1), kLoad,
              kNoCheckpoint);
    addMatrix(MI::kDzudzn, MT::kReal, DimensionSizes(1, 1, fullDims.nz), kLoad,
              kNoCheckpoint, isSimulation3D);

    addMatrix(MI::kDxudxnSgx, MT::kReal, DimensionSizes(fullDims.nx, 1, 1),
              kLoad, kNoCheckpoint);
    addMatrix(MI::kDyudynSgy, MT::kReal, DimensionSizes(1, fullDims.ny, 1),
              kLoad, kNoCheckpoint);
    addMatrix(MI::kDzudznSgz, MT::kReal, DimensionSizes(1, 1, fullDims.nz),
              kLoad, kNoCheckpoint, isSimulation3D);
  }
  ...
  if (params.getSensorMaskType() == Parameters::SensorMaskType::kIndex) {
    addMatrix(MI::kSensorMaskIndex, MT::kIndex,
              DimensionSizes(params.getSensorMaskIndexSize(), 1, 1), kLoad,
              kNoCheckpoint);
  }
  ...
  if (params.getSensorMaskType() == Parameters::SensorMaskType::kCorners) {
    addMatrix(MI::kSensorMaskCorners, MT::kIndex,
              DimensionSizes(6, params.getSensorMaskCornersSize(), 1), kLoad,
              kNoCheckpoint);
  }
  ...
  if (params.getInitialPressureSourceFlag() == 1) {
    addMatrix(MI::kInitialPressureSourceInput, MT::kReal, fullDims, kLoad,
              kNoCheckpoint);
  }
  ...
  if ((params.getTransducerSourceFlag() != 0) ||
    ...
    addMatrix(MI::kVelocitySourceIndex, MT::kIndex,
              DimensionSizes(1, 1, params.getVelocitySourceIndexSize()), kLoad,
              kNoCheckpoint);
  }

  // Transducer source flag defined
  if (params.getTransducerSourceFlag() != 0) {
    addMatrix(MI::kDelayMask, MT::kIndex,
              DimensionSizes(1, 1, params.getVelocitySourceIndexSize()), kLoad,
              kNoCheckpoint);
    addMatrix(MI::kTransducerSourceInput, MT::kReal,
  ...
  if (params.getPressureSourceFlag() != 0) {
    if (params.getPressureSourceMany() == 0) {  // 1D case
      addMatrix(MI::kPressureSourceInput, MT::kReal,
                DimensionSizes(1, 1, params.getPressureSourceFlag()), kLoad,
                kNoCheckpoint);
    } else {  // 2D case
      addMatrix(MI::kPressureSourceInput, MT::kReal,
    ...
    addMatrix(MI::kPressureSourceIndex, MT::kIndex,
              DimensionSizes(1, 1, params.getPressureSourceIndexSize()), kLoad,
              kNoCheckpoint);
  }
  ...
  if (params.getVelocityXSourceFlag() != 0) {
    if (params.getVelocitySourceMany() == 0) {  // 1D
      addMatrix(MI::kVelocityXSourceInput, MT::kReal,
                DimensionSizes(1, 1, params.getVelocityXSourceFlag()), kLoad,
                kNoCheckpoint);
    } else {  // 2D
      addMatrix(MI::kVelocityXSourceInput, MT::kReal,
  ...
  if (params.getVelocityYSourceFlag() != 0) {
    if (params.getVelocitySourceMany() == 0) {  // 1D
      addMatrix(MI::kVelocityYSourceInput, MT::kReal,
                DimensionSizes(1, 1, params.getVelocityYSourceFlag()), kLoad,
                kNoCheckpoint);
    } else {  // 2D
      addMatrix(MI::kVelocityYSourceInput, MT::kReal,
                DimensionSizes(1, params.getVelocitySourceIndexSize(),
    ...
    if (params.getVelocityZSourceFlag() != 0) {
      if (params.getVelocitySourceMany() == 0) {  // 1D
        addMatrix(MI::kVelocityZSourceInput, MT::kReal,
                  DimensionSizes(1, 1, params.getVelocityZSourceFlag()), kLoad,
                  kNoCheckpoint);
      } else {  // 2D
        addMatrix(MI::kVelocityZSourceInput, MT::kReal,
                  DimensionSizes(1, params.getVelocitySourceIndexSize(),
  ...
  if (((params.getVelocitySourceMode() == Parameters::SourceMode::kAdditive) ||
      ...
      (params.getPressureSourceFlag() || params.getVelocityXSourceFlag() ||
       params.getVelocityYSourceFlag() || params.getVelocityZSourceFlag())) {
  ...
  if (params.getStoreVelocityNonStaggeredRawFlag()) {
    ...
    const size_t zCutSize =
        (isSimulation3D) ? fullDims.nx * fullDims.ny * nzR : 1;

    if ((xCutSize >= yCutSize) &&
        (xCutSize >= zCutSize)) {  // X cut is the biggest
      shiftDims.nx = nxR;
    } else if ((yCutSize >= xCutSize) &&
               (yCutSize >= zCutSize)) {  // Y cut is the biggest
      shiftDims.ny = nyR;
    } else if ((zCutSize >= xCutSize) &&
               (zCutSize >= yCutSize)) {  // Z cut is the biggest
      shiftDims.nz = nzR;
    } else {  // all are the same
      shiftDims.nx = nxR;
    }

    addMatrix(MI::kTempFftwShift, MT::kFftwComplex, shiftDims, kNoLoad,
              kNoCheckpoint);
    ...
    addMatrix(MI::kUzShifted, MT::kReal, fullDims, kNoLoad, kNoCheckpoint,
              isSimulation3D);

    // shifts from the input file
    addMatrix(MI::kXShiftNegR, MT::kComplex, DimensionSizes(nxR, 1, 1), kNoLoad,
              kNoCheckpoint);
    addMatrix(MI::kYShiftNegR, MT::kComplex, DimensionSizes(1, nyR, 1), kNoLoad,
              kNoCheckpoint);
    addMatrix(MI::kZShiftNegR, MT::kComplex, DimensionSizes(1, 1, nzR), kNoLoad,
              kNoCheckpoint, isSimulation3D);
  ...
  addMatrix(MI::kTemp1RealND, tempRealMatrixType, fullDims, kNoLoad,
            kNoCheckpoint);
  // This one is necessary for absorption
  addMatrix(MI::kTemp2RealND, tempRealMatrixType, fullDims, kNoLoad,
            kNoCheckpoint);
  ...
  if ((params.getAbsorbingFlag() != Parameters::AbsorptionType::kLossless) &&
    ...
    // This special case has to be done using the se method since the
    // AlphaCoeffName has no matrix
    addMatrix(MI::kTemp3RealND, tempRealMatrixType, fullDims, kLoad,
              kNoCheckpoint);
  } else {
    addMatrix(MI::kTemp3RealND, tempRealMatrixType, fullDims, kNoLoad,
              kNoCheckpoint);
  }

  addMatrix(MI::kTempFftwX, MT::kFftwComplex, reducedDims, kNoLoad,
            kNoCheckpoint);
  addMatrix(MI::kTempFftwY, MT::kFftwComplex, reducedDims, kNoLoad,
            kNoCheckpoint);
  addMatrix(MI::kTempFftwZ, MT::kFftwComplex, reducedDims, kNoLoad,
            kNoCheckpoint, isSimulation3D);
}  // end of init
//----------------------------------------------------------------------------------------------------------------------
...
void MatrixContainer::createMatrices() {
  ...
  for (auto& it : mContainer) {
    if (it.second.matrixPtr != nullptr) {  // the data is already allocated
      throw std::invalid_argument(Logger::formatMessage(
          kErrFmtRelocationError, it.second.matrixName.c_str()));
    }
    ...
    switch (it.second.matrixType) {
      ...
      default: {  // unknown matrix type
        throw std::invalid_argument(Logger::formatMessage(
            kErrFmtBadMatrixType, it.second.matrixName.c_str()));
        break;
      }
    }  // switch
  }  // end for
...
void MatrixContainer::storeDataIntoCheckpointFile() {
  ...
  for (const auto& it : mContainer) {
    if (it.second.checkpoint) {
      it.second.matrixPtr->writeData(checkpointFile, it.second.matrixName,
                                     compressionLevel);
    }
  }
