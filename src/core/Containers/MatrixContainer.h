 ...
 * @details This container is responsible for maintaining all the matrices in
 * the code except the output streams. The matrices are allocated, freed, loaded
 * stored and check-pointed from here.
 */
class MatrixContainer {
 public:
  /**
   * @enum  MatrixIdx
   * @brief Matrix identifers of all matrices in the 2D, 3D and axisymmetric
   * fluid k-space code.
   */
  ...
  /**
   * @brief   Operator [].
   * @details The const version is not offered since the container is mutable
   * and one could modify records in the container.
   ...
   * @details This template routine returns the reference to the matrix
   * re-casted to the specific class type.
   *
  ...
  /**
   * @brief   Get the matrix with a specific type from the const container. The
   * matrix is mutable.
   * @details This template routine returns the reference to the matrix
   * re-casted to the specific class type.
   *
 ...
 protected:
 private:
  /// Container holding all the names of all possible matrices present in the
  /// input file
  static std::map<MatrixIdx, MatrixName> sMatrixHdf5Names;
  /// Map holding the container, it is mutable since we want to modify data in
  /// matrices within the container.
