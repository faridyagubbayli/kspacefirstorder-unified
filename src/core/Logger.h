 ...
 * @details Static class used for printing out info and error message based on
...
class Logger {
 public:
  ...
  /**
   * @brief   Template wrapper to log desired activity for a given log level
   * with formating string.
   * @details The routine takes a std::string as a formatting string for printf
   ...
   * @param   [in] args       - Arguments, std::string is not accepted and must
   * be passed as char*.
   */
  template <typename... Args>
  static void log(const LogLevel queryLevel, const std::string& format,
                  Args... args) {
  ...
  /**
   * @brief Log desired activity for a given log level without a formating
   * string.
  ...
  /**
   * @brief Wrap the line in the string based on delimiters and align it with
   * the rest of the logger output.
   *
  ...
  /**
   * @brief   C++-11 replacement for sprintf that works with std::string instead
   * of char*
