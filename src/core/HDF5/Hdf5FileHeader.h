 ...
 * output or checkpoint file header.
 */
class Hdf5FileHeader {
 public:
  /**
  ...
  /**
   * @brief   Read header from the output file (necessary for
   * checkpoint-restart).
   * @details Read only execution times (the others are read from the input
   * file, or calculated based on the very last leg of the simulation). This
   * function is called only if checkpoint-restart is enabled.
   *
  ...
  /**
   * @brief Write header to the output file (only a subset of all possible
   * fields are written).
   *
   * @param [in, out] checkpointFile - Checkpoint file handle.
   * @throw ios:failure              - If error happens.
  ...
  /**
   * @brief  Get string representing of file version of the actually opened
   * input file.
   ...
   */
  void getExecutionTimes(double& totalTime, double& loadTime,
                         double& preProcessingTime, double& simulationTime,
 ...
 private:
   ...
   * @brief  Structure holding the string representation of the file major and
   * minor version.
   */
  struct FileVersionNames {
  ...
  /// Current file version. Cannot be initialized here due to g++ compiler
  /// problems.
