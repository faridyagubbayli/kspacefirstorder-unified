 ...
 */
std::map<Hdf5File::MatrixDomainType, std::string>
    Hdf5File::sMatrixDomainTypeNames{
 ...
 * Constructor.
 */
Hdf5File::Hdf5File() : mFile(H5I_BADID), mFileName("") {}  // end of constructor
//----------------------------------------------------------------------------------------------------------------------
...
void Hdf5File::create(const string& fileName, unsigned int flags) {
  ...
  if (isOpen()) {
    throw ios::failure(
        Logger::formatMessage(kErrFmtCannotRecreateFile, fileName.c_str()));
  }
  ...
  if (mFile < 0) {
    throw ios::failure(
        Logger::formatMessage(kErrFmtCannotCreateFile, fileName.c_str()));
  }
}  // end of create
//----------------------------------------------------------------------------------------------------------------------
...
void Hdf5File::open(const string& fileName, unsigned int flags) {
  ...
  if (isOpen()) {
    throw ios::failure(
        Logger::formatMessage(kErrFmtCannotReopenFile, cFileName));
  };
...
void Hdf5File::close() {
  ...
  if (status < 0) {
    throw ios::failure(
        Logger::formatMessage(kErrFmtCannotCloseFile, mFileName.c_str()));
  }
...
hid_t Hdf5File::createGroup(const hid_t parentGroup,
  ...
  hid_t group = H5Gcreate(parentGroup, groupName.c_str(), H5P_DEFAULT,
                          H5P_DEFAULT, H5P_DEFAULT);

  if (group == H5I_INVALID_HID) {
    throw ios::failure(Logger::formatMessage(
        kErrFmtCannotCreateGroup, groupName.c_str(), mFileName.c_str()));
  }
...
hid_t Hdf5File::openGroup(const hid_t parentGroup,
  ...
  if (group == H5I_INVALID_HID) {
    throw ios::failure(Logger::formatMessage(
        kErrFmtCannotOpenGroup, groupName.c_str(), mFileName.c_str()));
  }
...
hid_t Hdf5File::createDataset(const hid_t parentGroup,
  ...
  // 4D dataset
  if (dimensionSizes.is4D()) {  // 4D dataset
  ...
  if (status < 0) {
    throw ios::failure(Logger::formatMessage(
        kErrFmtCannotOpenDataset, mFileName.c_str(), datasetName.c_str()));
  }

  // Set compression level
  status =
      H5Pset_deflate(propertyList, static_cast<unsigned int>(compressionLevel));
  if (status < 0) {
    throw ios::failure(
        Logger::formatMessage(kErrFmtCannotSetCompression, mFileName.c_str(),
                              datasetName.c_str(), compressionLevel));
  }

  const hid_t datasetType = (matrixDataType == MatrixDataType::kFloat)
                                ? H5T_NATIVE_FLOAT
                                : H5T_STD_U64LE;

  // Create dataset
  hid_t dataset = H5Dcreate(parentGroup, datasetName.c_str(), datasetType,
                            dataspace, H5P_DEFAULT, propertyList, H5P_DEFAULT);
...
void Hdf5File::writeHyperSlab(const hid_t dataset,
  ...
  if (std::is_same<T, size_t>()) {
    status = H5Dwrite(dataset, H5T_STD_U64LE, memspace, filespace, H5P_DEFAULT,
                      data);
  }
...
void Hdf5File::writeCuboidToHyperSlab(const hid_t dataset,
  ...
  hsize_t offsetInDataset[rank] = {hyperslabPosition.nt, hyperslabPosition.nz,
                                   hyperslabPosition.ny, hyperslabPosition.nx};
  hsize_t offsetInMatrixData[] = {cuboidPosition.nz, cuboidPosition.ny,
                                  cuboidPosition.nx};
  hsize_t matrixSize[] = {matrixDimensions.nz, matrixDimensions.ny,
                          matrixDimensions.nx};

  // Select hyperslab in the HDF5 dataset
  filespace = H5Dget_space(dataset);
  status = H5Sselect_hyperslab(filespace, H5S_SELECT_SET, offsetInDataset,
                               nullptr, slabSize, nullptr);
  if (status < 0) {
    throw ios::failure(Logger::formatMessage(kErrFmtCannotWriteDataset, ""));
  }
  ...
  status =
      H5Sselect_hyperslab(memspace, H5S_SELECT_SET, offsetInMatrixData, nullptr,
                          slabSize + 1,  // Slab size has to be 3D in this case
                                         // (done by skipping the T dimension)
                          nullptr);
  if (status < 0) {
    throw ios::failure(Logger::formatMessage(kErrFmtCannotWriteDataset, ""));
  }
  ...
  // Write the data
  status = H5Dwrite(dataset, H5T_NATIVE_FLOAT, memspace, filespace, H5P_DEFAULT,
                    matrixData);
  if (status < 0) {
    throw ios::failure(Logger::formatMessage(kErrFmtCannotWriteDataset, ""));
  }
 ...
 */
void Hdf5File::writeSensorByMaskToHyperSlab(
    const hid_t dataset, const DimensionSizes& hyperslabPosition,
    const size_t indexSensorSize, const size_t* indexSensorData,
    const DimensionSizes& matrixDimensions, const float* matrixData) {
  herr_t status;
  hid_t filespace, memspace;
  ...
  hsize_t offsetInDataset[rank] = {hyperslabPosition.nz, hyperslabPosition.ny,
                                   hyperslabPosition.nx};

  // Treat as a 1D array
  hsize_t matrixSize =
      matrixDimensions.nz * matrixDimensions.ny * matrixDimensions.nx;
  ...
  status = H5Sselect_elements(memspace, H5S_SELECT_SET, indexSensorSize,
...
void Hdf5File::writeScalarValue(const hid_t parentGroup,
                                const MatrixName& datasetName, const T value) {
  constexpr int rank = 3;
  ...
  if (H5LTfind_dataset(parentGroup, cDatasetName) ==
      1) {  // Dataset already exists (from previous simulation leg) open it
    ...
    dataset = H5Dcreate(parentGroup, cDatasetName, datatype, dataspace,
  ...
  if (dataset == H5I_INVALID_HID) {
    throw ios::failure(
        Logger::formatMessage(kErrFmtCannotWriteDataset, cDatasetName));
  }
...
/**
 *  Write a scalar value at a specified place in the file tree, float value
 * explicit instance.
 */
...
//----------------------------------------------------------------------------------------------------------------------
/**
 * Write a scalar value at a specified place in the file tree, index value
 * explicit instance.
 */
...
void Hdf5File::readScalarValue(const hid_t parentGroup,
                               const MatrixName& datasetName, T& value) {
  readCompleteDataset(parentGroup, datasetName, DimensionSizes(1, 1, 1),
                      &value);
}  // end of readScalarValue
//----------------------------------------------------------------------------------------------------------------------

/**
 * Read a scalar value at a specified place in the file tree, float value,
 * explicit instance.
 */
...
void Hdf5File::readCompleteDataset(const hid_t parentGroup,
  ...
  if (getDatasetDimensionSizes(parentGroup, datasetName).nElements() !=
    ...
    throw ios::failure(
        Logger::formatMessage(kErrFmtBadDimensionSizes, cDatasetName));
  }

  herr_t status = -1;
  if (std::is_same<T, float>()) {
    status =
        H5LTread_dataset(parentGroup, cDatasetName, H5T_NATIVE_FLOAT, data);
  }
  if (std::is_same<T, size_t>()) {
  ...
  if (status < 0) {
    throw ios::failure(
        Logger::formatMessage(kErrFmtCannotReadDataset, cDatasetName));
  }
}  // end of readCompleteDataset
//----------------------------------------------------------------------------------------------------------------------

/**
 * Read data from the dataset at a specified place in the file tree, float
 * version explicit instance.
 */
template void Hdf5File::readCompleteDataset<float>(
    const hid_t parentGroup, const MatrixName& datasetName,
    const DimensionSizes& dimensionSizes, float* data);
//----------------------------------------------------------------------------------------------------------------------
/**
 * Read data from the dataset at a specified place in the file tree, index
 * version explicit instance.
 */
template void Hdf5File::readCompleteDataset<size_t>(
    const hid_t parentGroup, const MatrixName& datasetName,
    const DimensionSizes& dimensionSizes, size_t* data);
//----------------------------------------------------------------------------------------------------------------------

/**
 * Get dimension sizes of the dataset at a specified place in the file tree.
 */
DimensionSizes Hdf5File::getDatasetDimensionSizes(
    const hid_t parentGroup, const MatrixName& datasetName) {
  const size_t ndims = getDatasetNumberOfDimensions(parentGroup, datasetName);

  hsize_t dims[4] = {0, 0, 0, 0};

  herr_t status = H5LTget_dataset_info(parentGroup, datasetName.c_str(), dims,
                                       nullptr, nullptr);
  if (status < 0) {
    throw ios::failure(
        Logger::formatMessage(kErrFmtCannotReadDataset, datasetName.c_str()));
  }
...
/**
 * Get number of dimensions of the dataset at a specified place in the file
 * tree.
 */
size_t Hdf5File::getDatasetNumberOfDimensions(const hid_t parentGroup,
  ...
  herr_t status =
      H5LTget_dataset_ndims(parentGroup, datasetName.c_str(), &dims);
  if (status < 0) {
    throw ios::failure(
        Logger::formatMessage(kErrFmtCannotReadDataset, datasetName.c_str()));
  }
...
/**
 * Write matrix data type into the dataset at a specified place in the file
 * tree.
 */
void Hdf5File::writeMatrixDataType(const hid_t parentGroup,
                                   const MatrixName& datasetName,
                                   const MatrixDataType& matrixDataType) {
  writeStringAttribute(parentGroup, datasetName, kMatrixDataTypeName,
                       sMatrixDataTypeNames[matrixDataType]);
}  // end of writeMatrixDataType
//----------------------------------------------------------------------------------------------------------------------

/**
 * Write matrix domain type into the dataset at a specified place in the file
 * tree.
 */
void Hdf5File::writeMatrixDomainType(const hid_t parentGroup,
                                     const MatrixName& datasetName,
                                     const MatrixDomainType& matrixDomainType) {
  writeStringAttribute(parentGroup, datasetName, kMatrixDomainTypeName,
 ...
 */
Hdf5File::MatrixDataType Hdf5File::readMatrixDataType(
    const hid_t parentGroup, const MatrixName& datasetName) {
  const string paramValue =
      readStringAttribute(parentGroup, datasetName, kMatrixDataTypeName);
  ...
  throw ios::failure(
      Logger::formatMessage(kErrFmtBadAttributeValue, datasetName.c_str(),
 ...
 */
Hdf5File::MatrixDomainType Hdf5File::readMatrixDomainType(
    const hid_t parentGroup, const MatrixName& datasetName) {
  const string paramValue =
      readStringAttribute(parentGroup, datasetName, kMatrixDomainTypeName);
...
void Hdf5File::writeStringAttribute(const hid_t parentGroup,
                                    ...
                                    const string& value) {
  herr_t status = H5LTset_attribute_string(
      parentGroup, datasetName.c_str(), attributeName.c_str(), value.c_str());
  if (status < 0) {
...
string Hdf5File::readStringAttribute(const hid_t parentGroup,
                                     ...
                                     const MatrixName& attributeName) {
  char value[256] = "";
  herr_t status = H5LTget_attribute_string(parentGroup, datasetName.c_str(),
