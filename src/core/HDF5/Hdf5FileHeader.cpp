 ...
 *
 * @brief     The implementation of the class responsible for working with file
 * headers.
 *
 ...
 */
const Hdf5FileHeader::FileVersion Hdf5FileHeader::kCurrentFileVersion =
    Hdf5FileHeader::FileVersion::kVersion12;

/**
 * Initialization of static map with header attribute names.
 */
std::map<Hdf5FileHeader::FileHeaderItems, std::string>
    Hdf5FileHeader::sHeaderNames{
        ...
        {Hdf5FileHeader::FileHeaderItems::kNumberOfCores,
         "number_of_cpu_cores"},
        {Hdf5FileHeader::FileHeaderItems::kTotalMemoryConsumption,
         "total_memory_in_use"},
        {Hdf5FileHeader::FileHeaderItems::kPeakMemoryConsumption,
         "peak_core_memory_in_use"},

        {Hdf5FileHeader::FileHeaderItems::kTotalExecutionTime,
         "total_execution_time"},
        {Hdf5FileHeader::FileHeaderItems::kDataLoadTime,
         "data_loading_phase_execution_time"},
        {Hdf5FileHeader::FileHeaderItems::kPreProcessingTime,
         "pre-processing_phase_execution_time"},
        {Hdf5FileHeader::FileHeaderItems::kSimulationTime,
         "simulation_phase_execution_time"},
        {Hdf5FileHeader::FileHeaderItems::kPostProcessingTime,
         "post-processing_phase_execution_time"}};  // end of sHeaderNames
//----------------------------------------------------------------------------------------------------------------------
...
std::map<Hdf5FileHeader::FileType, std::string> Hdf5FileHeader::sFileTypesNames{
    ...
    {Hdf5FileHeader::FileType::kUnknown, "unknown"}};  // end of sHeaderNames
//----------------------------------------------------------------------------------------------------------------------

/**
 * Initialization of file version names
 */
std::map<Hdf5FileHeader::FileVersion, Hdf5FileHeader::FileVersionNames>
    Hdf5FileHeader::sFileVersionNames{
        {Hdf5FileHeader::FileVersion::kVersion10,
         Hdf5FileHeader::FileVersionNames{"1", "0"}},
        {Hdf5FileHeader::FileVersion::kVersion11,
         Hdf5FileHeader::FileVersionNames{"1", "1"}},
        {Hdf5FileHeader::FileVersion::kVersion12,
         Hdf5FileHeader::FileVersionNames{"1", "2"}},
 ...
 * Constructor.
 */
Hdf5FileHeader::Hdf5FileHeader() : mHeaderValues() {}  // end of constructor
//----------------------------------------------------------------------------------------------------------------------
...
Hdf5FileHeader::Hdf5FileHeader(const Hdf5FileHeader& src)
    : mHeaderValues(src.mHeaderValues) {}  // end of copy constructor
//----------------------------------------------------------------------------------------------------------------------
...
void Hdf5FileHeader::readHeaderFromInputFile(Hdf5File& inputFile) {
  ...
  auto readItem = [this, &inputFile, &rootGroup](FHI item) {
    mHeaderValues[item] =
        inputFile.readStringAttribute(rootGroup, "/", sHeaderNames[item]);
  };  // readItem
...
void Hdf5FileHeader::readHeaderFromOutputFile(Hdf5File& outputFile) {
  ...
  auto readItem = [this, &outputFile, &rootGroup](FHI item) {
    mHeaderValues[item] =
        outputFile.readStringAttribute(rootGroup, "/", sHeaderNames[item]);
  };  // readItem
...
void Hdf5FileHeader::readHeaderFromCheckpointFile(Hdf5File& checkpointFile) {
  ...
  auto readItem = [this, &checkpointFile, &rootGroup](FHI item) {
    mHeaderValues[item] =
        checkpointFile.readStringAttribute(rootGroup, "/", sHeaderNames[item]);
  };  // readItem
...
void Hdf5FileHeader::writeHeaderToOutputFile(Hdf5File& outputFile) {
  ...
  for (const auto& it : sHeaderNames) {
    outputFile.writeStringAttribute(rootGroup, "/", it.second,
                                    mHeaderValues[it.first]);
  }
}  // end of writeHeaderToOutputFile
//----------------------------------------------------------------------------------------------------------------------

/**
 * Write header to the output file (only a subset of all possible fields are
 * written).
 */
void Hdf5FileHeader::writeHeaderToCheckpointFile(Hdf5File& checkpointFile) {
  ...
  auto writeItem = [this, &checkpointFile, &rootGroup](FHI item) {
    checkpointFile.writeStringAttribute(rootGroup, "/", sHeaderNames[item],
                                        mHeaderValues[item]);
  };  // readItem
...
void Hdf5FileHeader::setActualCreationTime() {
  ...
  mHeaderValues[FileHeaderItems::kCreationDate] =
      Logger::formatMessage("%02i/%02i/%02i, %02i:%02i:%02i", current->tm_mday,
                            current->tm_mon + 1, current->tm_year - 100,
...
std::string Hdf5FileHeader::getFileVersionName() {
  return mHeaderValues[FileHeaderItems::kMajorVersion] + "." +
...
void Hdf5FileHeader::setMajorFileVersion() {
  mHeaderValues[FileHeaderItems::kMajorVersion] =
      sFileVersionNames[kCurrentFileVersion].majorVersion;
...
Hdf5FileHeader::FileVersion Hdf5FileHeader::getFileVersion() {
  for (const auto& it : sFileVersionNames) {
    if ((it.second.majorVersion ==
         mHeaderValues[FileHeaderItems::kMajorVersion]) &&
        (it.second.minorVersion ==
         mHeaderValues[FileHeaderItems::kMinorVersion])) {
      return it.first;
    }
  }
...
// Windows build
  ...
  mHeaderValues[FileHeaderItems::kHostName] = Logger::formatMessage(
      "%s (%s)", hostName,
      Parameters::getInstance().getProcessorName().c_str());
}  // end of setHostName
//----------------------------------------------------------------------------------------------------------------------
...
void Hdf5FileHeader::setMemoryConsumption(const size_t totalMemory) {
  mHeaderValues[FileHeaderItems::kTotalMemoryConsumption] =
      Logger::formatMessage("%ld MB", totalMemory);
 ...
 */
void Hdf5FileHeader::getExecutionTimes(double& totalTime, double& loadTime,
...
void Hdf5FileHeader::setNumberOfCores() {
  mHeaderValues[FileHeaderItems::kNumberOfCores] = Logger::formatMessage(
      "%ld", Parameters::getInstance().getNumberOfThreads());
