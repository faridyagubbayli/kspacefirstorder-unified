 ...
 *
 * @brief     The header file containing the class for 64b integer index
 * matrices.
 *
...
/**
 * @class   IndexMatrix
 * @brief   The class for 64b unsigned integers (indices). It is used for linear
 * and cuboid corners masks to get the address of sampled voxels.
 *
 * @details The class for 64b unsigned integers (indices). It is used for linear
 * and cuboid corners masks to get the get the address of sampled voxels.
 */
class IndexMatrix : public BaseIndexMatrix {
 public:
   ...
   */
  virtual void readData(Hdf5File& file, const MatrixName& matrixName) override;
  /**
   ...
   */
  virtual void writeData(Hdf5File& file, const MatrixName& matrixName,
                         const size_t compressionLevel) override;

  /**
   * @brief   Operator [].
   * @details This operator is not used in the code due to persisting
   * performance issues with vectorization.
  ...
  /**
   * @brief  Get total number of elements in all cuboids to be able to allocate
   * output file.
