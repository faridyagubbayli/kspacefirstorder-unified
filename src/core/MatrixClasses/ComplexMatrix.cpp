 ...
 * Destructor.
 */
ComplexMatrix::~ComplexMatrix() { freeMemory(); }  // end of ~ComplexMatrix
//----------------------------------------------------------------------------------------------------------------------

/**
 * Read data from HDF5 file (do some basic checks). Only from the root group.
 */
void ComplexMatrix::readData(Hdf5File& file, const MatrixName& matrixName) {
  // Check data type
  ...
  if (file.readMatrixDomainType(file.getRootGroup(), matrixName) !=
    ...
    throw std::ios::failure(
        Logger::formatMessage(kErrFmtMatrixNotComplex, matrixName.c_str()));
  }
 ...
 */
void ComplexMatrix::writeData(Hdf5File& file, const MatrixName& matrixName,
  ...
  // Create a dataset
  hid_t dataset =
      file.createDataset(file.getRootGroup(), matrixName, complexDims, chunks,
