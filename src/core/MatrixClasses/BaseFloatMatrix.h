 ...
 *
 * @brief     The header file containing the base class for single precisions
 ...
 * @brief   Abstract base class for float based matrices defining basic
 * interface. Higher dimensional matrices stored as 1D arrays, row-major order.
 *
 * @details Abstract base class for float based matrices defining basic
 * interface. Higher dimensional matrices stored as 1D arrays, row-major order.
 * The I/O is done via HDF5 files.
 */
class BaseFloatMatrix : public BaseMatrix {
 public:
   ...
   */
  virtual const DimensionSizes& getDimensionSizes() const override {
  ...
  /**
   * @brief  The capacity of the matrix (this may differ from size due to
   * padding, etc.).
  ...
  /**
   * @brief Get matrix data out of the class (for direct kernel access), const
   * version.
