 ...
 * Destructor.
 */
RealMatrix::~RealMatrix() { freeMemory(); }  // end of ~RealMatrix
//----------------------------------------------------------------------------------------------------------------------

/**
 * Read data data from HDF5 file (only from the root group).
 */
void RealMatrix::readData(Hdf5File& file, const MatrixName& matrixName) {
  // Test matrix datatype
  if (file.readMatrixDataType(file.getRootGroup(), matrixName) !=
      Hdf5File::MatrixDataType::kFloat) {
    throw std::ios::failure(
        Logger::formatMessage(kErrFmtMatrixNotFloat, matrixName.c_str()));
  }
 ...
 */
void RealMatrix::writeData(Hdf5File& file, const MatrixName& matrixName,
  ...
  hid_t dataset = file.createDataset(
      ...
      Hdf5File::MatrixDataType::kFloat, compressionLevel);
