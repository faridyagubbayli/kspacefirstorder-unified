 ...
 *
 * @brief     The implementation file containing the class for 64b integer index
 * matrices.
 *
 ...
 * Destructor.
 */
IndexMatrix::~IndexMatrix() { freeMemory(); }  // end of ~IndexMatrix
//----------------------------------------------------------------------------------------------------------------------

/**
 * Read data from HDF5 file (only from the root group).
 */
void IndexMatrix::readData(Hdf5File& file, const MatrixName& matrixName) {
  // Check the datatype
  if (file.readMatrixDataType(file.getRootGroup(), matrixName) !=
      Hdf5File::MatrixDataType::kIndex) {
    throw std::ios::failure(
        Logger::formatMessage(kErrFmtMatrixNotIndex, matrixName.c_str()));
  }
  ...
  if (file.readMatrixDomainType(file.getRootGroup(), matrixName) !=
    ...
    throw std::ios::failure(
        Logger::formatMessage(kErrFmtMatrixNotReal, matrixName.c_str()));
  }

  // Read data
  file.readCompleteDataset(file.getRootGroup(), matrixName, mDimensionSizes,
                           mData);
}  // end of readData
//----------------------------------------------------------------------------------------------------------------------
 ...
 */
void IndexMatrix::writeData(Hdf5File& file, const MatrixName& matrixName,
  ...
  if ((mDimensionSizes.ny == 1) && (mDimensionSizes.nz == 1)) {
    if (mDimensionSizes.nx >
        4 * kChunkSize1D8MB) {  // Chunk = 8MB for big matrices (> 32MB).
      chunks.nx = kChunkSize1D8MB;
    } else if (mDimensionSizes.nx >
               4 * kChunkSize1D1MB) {  // Chunk = 1MB for big matrices (> 4 -
                                       // 32MB).
      chunks.nx = kChunkSize1D1MB;
    } else if (mDimensionSizes.nx >
               4 * kChunkSize1D128kB) {  // Chunk = 128kB for big matrices (<
                                         // 1MB).
  ...
  hid_t dataset = file.createDataset(
      file.getRootGroup(), matrixName, mDimensionSizes, chunks,
      Hdf5File::MatrixDataType::kIndex, compressionLevel);
  ...
  file.writeMatrixDataType(file.getRootGroup(), matrixName,
                           Hdf5File::MatrixDataType::kIndex);
  file.writeMatrixDomainType(file.getRootGroup(), matrixName,
                             Hdf5File::MatrixDomainType::kReal);
}  // end of writeData
//----------------------------------------------------------------------------------------------------------------------
 ...
 * Cuboids are stored as 6-tuples (two 3D coordinates). This gives the first
 * three coordinates.
 */
...
/**
 * Get total number of elements in all cuboids to be able to allocate output
 * file.
 */
size_t IndexMatrix::getSizeOfAllCuboids() const {
  ...
  for (size_t cuboidIdx = 0; cuboidIdx < mDimensionSizes.ny; cuboidIdx++) {
    elementSum +=
        (getBottomRightCorner(cuboidIdx) - getTopLeftCorner(cuboidIdx))
            .nElements();
  }
