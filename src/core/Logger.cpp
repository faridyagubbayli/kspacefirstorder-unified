 ...
 *
 * @brief     The implementation file containing a class responsible for
 * printing out info and error messages (stdout, and stderr).
 *
...
/**
 * Wrap the line based on delimiters and align it with the rest of the logger
 * output.
 */
string Logger::wordWrapString(const string& inputString,
                              const string& delimiters, const int indentation,
                              const int lineSize) {
  ...
  while (textStream.good()) {
    ...
    if (spaceLeft <
        static_cast<int>(word.length()) + 3) {  // Fill the end of the line
