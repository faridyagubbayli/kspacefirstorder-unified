...
#ifdef USE_CUDA
// CUDA backend uses CufftComplexMatrix instead of FftwComplexMatrix
#include <backends/cuda/MatrixClasses/CufftComplexMatrix.h>
// Define aliases for CUDA matrices
using FftwComplexMatrix = CufftComplexMatrix;
using FftwRealMatrix = RealMatrix;  // CUDA uses RealMatrix directly
#else
// Default OpenMP backend
#include <backends/openmp/MatrixClasses/FftwComplexMatrix.h>
#include <backends/openmp/MatrixClasses/FftwRealMatrix.h>
#endif
 ...
 * @brief   Class responsible for solving the ultrasound propagation in fluid
 * medium.
 * @details The simulation is based on the k-space first order method. The
 * solver supports 2D normal and AS medium, and 3D normal medium. This class
 * maintain the whole k-wave (implements the time loop).
 */
class KSpaceFirstOrderSolver {
 public:
  /// Constructor.
   ...
   * @details If checkpointing is enabled, this may include reading data from
   ...
   * @details It first initializes FFTs, then runs the pre-processing phase,
   * continues through the simulation time loop and applies post-process on the
   * data.
   */
 ...
 protected:
  ...
  /// Read the header of the output file and sets the cumulative elapsed time
  /// from the first log.
   ...
   * @details Initialize all indices, pre-compute constants such as c^2, rho0Sgx
   * * dt  and create kappa, derivative and shift operators, PML, absorbEta,
   * absorbTau, absorbNabla1, absorbNabla2  matrices.
   ...
   */
  template <Parameters::SimulationDimension simulationDimension,
            bool rho0ScalarFlag, bool bOnAScalarFlag, bool c0ScalarFlag,
   ...
   * @details Results dp/dx, dp/dy and dp/dz are stored in kTemp1RealND,
   * kTemp1RealND and kTemp1RealND.
   *
   * @tparam  simulationDimension - Dimensionality of the simulation.
   *
   ...
   * @details Results dp/dx and dp/dy are stored in kTemp1RealND and
   * kTemp1RealND.
   *
   * <b>Matlab code:</b> \code
   *  p_k = fft(dtt1D(p, DCT3, 2), [], 1);
   *
   *  kTemp1RealND = dtt1D(real(ifft(bsxfun(@times, ddx_k_shift_pos, kappa .*
   * p_k), [], 1)), DCT2, 2) ./ M; kTemp2RealND = dtt1D(real(ifft(bsxfun(@times,
   * ddy_k_wswa, kappa .* p_k), [], 1)), DST4, 2) ./ M;
   * \endcode
   *
   */
  ...
  //------------------------------------------------//
  /**
   * @brief   Compute new values of acoustic velocity in all used dimensions
   * (UxSgx, UySgy, UzSgz).
   * @details This routine is used in both normal and axisymmetric medium. The
   * pressure gradients p_k or dpdx_sgx and dpdy_sgy on staggered grids are
   * stored in kTemp1RealND, kTemp2RealND, kTemp3RealND variables.
   *
   * @tparam  simulationDimension - Dimensionality of the simulation.
   * @tparam  rho0ScalarFlag      - Is density homogeneous?
   *
   ...
   * .* rho0_sgx_inv .* kTemp1RealND); uy_sgy = bsxfun(@times, pml_y_sgy,
   * bsxfun(@times, pml_y_sgy, uy_sgy) - dt .* rho0_sgy_inv .* kTemp2RealND);
   *  uz_sgz = bsxfun(@times, pml_z_sgz, bsxfun(@times, pml_z_sgz, uz_sgz) - dt
   * .* rho0_sgz_inv .* kTemp3RealND);
   * \endcode
   */
  template <Parameters::SimulationDimension simulationDimension,
            bool rho0ScalarFlag>
   ...
   *  ux_sgx = bsxfun(@times, pml_x_sgx, bsxfun(@times, pml_x_sgx, ux_sgx) - dt
   * .* rho0_sgx_inv .* kTemp1RealND) uy_sgy = bsxfun(@times, pml_y_sgy,
   * bsxfun(@times, pml_y_sgy, uy_sgy) - dt .* rho0_sgy_inv .* kTemp1Rea2ND)
   *  uz_sgz = bsxfun(@times, pml_z_sgz, bsxfun(@times, pml_z_sgz, uz_sgz) - dt
   * .* rho0_sgz_inv .* kTemp1Rea3ND)
   *\endcode
   */
  template <Parameters::SimulationDimension simulationDimension,
            bool rho0ScalarFlag>
  ...
  /**
   * @brief  Compute acoustic velocity for homogenous medium and nonuniform
   * grid.
   * @tparam simulationDimension - Dimensionality of the simulation.
   *
   * <b> Matlab code: </b> \code
   ...
   *
   * <b>Matlab code:</b> \code
   *  ux_shifted = real(ifft(bsxfun(\@times, x_shift_neg, fft(ux_sgx, [], 1)),
   * [], 1)); uy_shifted = real(ifft(bsxfun(\@times, y_shift_neg, fft(uy_sgy,
   * [], 2)), [], 2)); uz_shifted = real(ifft(bsxfun(\@times, z_shift_neg,
   * fft(uz_sgz, [], 3)), [], 3));
   * \endcode
   */
  template <Parameters::SimulationDimension simulationDimension>
   ...
   * )); duydy = real(ifftn( bsxfun(@times, ddy_k_shift_neg, kappa .*
   * fftn(uy_sgy)) )); duzdz = real(ifftn( bsxfun(@times, ddz_k_shift_neg, kappa
   * .* fftn(uz_sgz)) ));
   * \endcode
   */
  template <Parameters::SimulationDimension simulationDimension>
  ...
  /**
   * @brief  Compute new values of acoustic velocity gradients, axisymmetric
   * medium, WSWA case symmetry.
   *
   * <b>Matlab code:</b> \code
   *  duxdx = dtt1D(real(ifft(...
   *                kappa .* bsxfun(@times, ddx_k_shift_neg, fft(dtt1D(ux_sgx,
   * DCT3, 2), [], 1)) ... , [], 1)), DCT2, 2) ./ M;
   *
   *  duydy = dtt1D(real(ifft(kappa .* (...
   *                bsxfun(@times, ddy_k_hahs, fft(dtt1D(uy_sgy, DST4, 2), [],
   * 1)) + ... fft(dtt1D(bsxfun(@times, 1./y_vec_sg, uy_sgy), DCT4, 2), [], 1)
   * ...
   *                ), [], 1)), DCT2, 2) ./ M;
   * \endcode
   */
   ...
   * duxdx); rhoy = bsxfun(@times, pml_y, bsxfun(@times, pml_y, rhoy) - dt .*
   * rho0 .* duydy); rhoz = bsxfun(@times, pml_z, bsxfun(@times, pml_z, rhoz) -
   * dt .* rho0 .* duzdz);
   * \endcode
   *
   */
   ...
   *
   * <b>Matlab code:</b> \code
   *  p = c.^2 .* (rhox + rhoy + rhoz + medium.BonA .* (rhox + rhoy + rhoz).^2
   * ./ (2 .* rho0));
   *\endcode
   */
  template <Parameters::SimulationDimension simulationDimension,
  ...
  /**
   * @brief  Compute acoustic pressure for nonlinear power law absorbing case,
   * normal medium.
   *
   * @tparam simulationDimension - Dimensionality of the simulation.
   * @tparam rho0ScalarFlag      - Is density homogeneous?
   ...
   *  p = c.^2 .* (...
   *      (rhox + rhoy + rhoz) ...
   *       + absorb_tau .* real(ifftn( absorb_nabla1 .* fftn(rho0 .* (duxdx +
   * duydy + duzdz)) ))...
  ...
  /**
   * @brief  Sum sub-terms to calculate new pressure with stokes absorption in
   * nonlinear case.
   *
   * @tparam simulationDimension - Dimensionality of the simulation.
   * @tparam rho0ScalarFlag      - Is density homogeneous?
  ...
  /**
   * @brief  Compute acoustic pressure for linear power law absorbing case,
   * normal medium.
   *
   * @tparam simulationDimension - Dimensionality of the simulation.
   * @tparam rho0ScalarFlag      - Is density homogeneous?
   ...
   * <b>Matlab code:</b> \code
   *  p = c.^2 .* ((rhox + rhoy + rhoz) ...
   *               + absorb_tau .* real(ifftn( absorb_nabla1 .* fftn(rho0 .*
   * (duxdx + duydy + duzdz)) )) ...
   *               - absorb_eta .* real(ifftn( absorb_nabla2 .* fftn(rhox + rhoy
   * + rhoz) )) ...
   *                );
   *\endcode
   */
  template <Parameters::SimulationDimension simulationDimension,
            bool rho0ScalarFlag, bool c0ScalarFlag, bool alphaCoefScalarFlag>
   ...
   *
   * <b>Matlab code:</b> \code*
   *  p = c.^2 .* ((rhox + rhoy + rhoz) + absorb_tau .* rho0 .* (duxdx + duydy +
   * duzdz));
   * \endcode
   */
  template <Parameters::SimulationDimension simulationDimension,
            bool rho0ScalarFlag, bool c0ScalarFlag, bool absorbTauScalarFlag>
  void sumPressureTermsLinearStokes();

  /**
   * @brief  Compute three temporary sums before taking FFTs in the power law
   * absorption during the nonlinear calculation of new pressure.
   *
   * @tparam simulationDimension      - Dimensionality of the simulation.
   * @tparam rho0ScalarFlag           - Is density homogeneous?
   * @tparam bOnAScalarFlag           - Is nonlinearity homogenous?
   ...
   */
  template <Parameters::SimulationDimension simulationDimension,
            bool rho0ScalarFlag, bool bOnAScalarFlag>
  void computePressureTermsNonlinearPowerLaw(RealMatrix& densitySum,
                                             RealMatrix& nonlinearTerm,
                                             RealMatrix& velocityGradientSum);
  /**
   * @brief  Compute two temporary sums before taking the FFT in the power law
   * absorption during the linear calculation of new pressure.
   *
   * @tparam simulationDimension      - Dimensionality of the simulation.
   * @tparam rho0ScalarFlag           - Is density homogeneous?
   *
  ...
  /**
   * @brief  Sum sub-terms after FFTs to calculate new pressure in the nonlinear
   * power law absorption case.
   *
   * @tparam c0ScalarFlag        - Is sound speed homogeneous?
   ...
   * @param [in] absorbTauTerm   - tau component.
   * @param [in] absorbEtaTerm   - eta component  of the pressure term.
   * @param [in] densitySum      - Sum of three components of density (rhoXSgx +
   * rhoYSgy + rhoZSgx).
   */
  template <bool c0ScalarFlag, bool tauAndEtaScalarFlag>
   ...
   *
   *  ux_sgx = dt .* rho0_sgx_inv .* real(ifftn( bsxfun(@times, ddx_k_shift_pos,
   * kappa .* fftn(p)) )) / 2; uy_sgy = dt .* rho0_sgy_inv .* real(ifftn(
   * bsxfun(@times, ddy_k_shift_pos, kappa .* fftn(p)) )) / 2; uz_sgz = dt .*
   * rho0_sgz_inv .* real(ifftn( bsxfun(@times, ddz_k_shift_pos, kappa .*
   * fftn(p)) )) / 2;
   * \endcode
   */
  template <Parameters::SimulationDimension simulationDimension,
            bool rho0ScalarFlag, bool c0ScalarFlag>
  ...
  /**
   * @brief  Compute acoustic velocity for initial pressure problem, homogenous
   * medium, nonuniform grid.
   * @tparam simulationDimension - Dimensionality of the simulation.
   *
   * <b> Matlab code: </b> \code
  ...
  /// Generate derivative operators for axisymmetric medium
  /// (ddx_k_shift_{pos,neg}, ddyKHahs, ddyKWswa, yVecSg).
  ...
  /// Generate absorbTau, absorbEta for heterogenous medium and power law
  /// absorption.
  ...
  //-------------------------------------------------//
  /**
   * @brief Compute 1D index using 3 spatial coordinates and the size of the
   * matrix.
   ...
   * @return
   */
  size_t get1DIndex(const size_t z, const size_t y, const size_t x,
                    const DimensionSizes& dimensionSizes) const;
  /**
   * @brief Compute 1D index using 2 spatial coordinates and the size of the
   * matrix.
   * @param [in] y              - y coordinate
   * @param [in] x              - x coordinate
   * @param [in] dimensionSizes - Size of the matrix.
   * @return
   */
  size_t get1DIndex(const size_t y, const size_t x,
                    const DimensionSizes& dimensionSizes) const;

  //--------------------------------------- Getters for temporary matrices
  //-----------------------------------------//
  /**
   * @brief  Get the first real 2D/3D temporary matrix. This matrix share memory
   * with temp1FftRealND.
   * @return Temporary real 2D/3D matrix.
   */
  RealMatrix& getTemp1RealND() {
    return mMatrixContainer.getMatrix<RealMatrix>(
        MatrixContainer::MatrixIdx::kTemp1RealND);
  };
  /**
   * @brief  Get the second real 2D/3D temporary matrix. This matrix share
   * memory with temp2FftRealND.
   * @return Temporary real 2D/3D matrix.
   */
  RealMatrix& getTemp2RealND() {
    return mMatrixContainer.getMatrix<RealMatrix>(
        MatrixContainer::MatrixIdx::kTemp2RealND);
  };
  /**
   * @brief  Get the third real 3D temporary matrix. This matrix is only present
   * for 3D simulations,
   * @return Temporary real 3D matrix.
   */
  RealMatrix& getTemp3RealND() {
    return mMatrixContainer.getMatrix<RealMatrix>(
        MatrixContainer::MatrixIdx::kTemp3RealND);
  };

  /**
   * @brief  Get the first temporary matrix for R2R fft. This matrix share
   * memory with temp1RealND.
   * @return Temporary real 2D matrix for axisymmetric code.
   */
  FftwRealMatrix& getTemp1FftwRealND() {
    return mMatrixContainer.getMatrix<FftwRealMatrix>(
        MatrixContainer::MatrixIdx::kTemp1RealND);
  };
  /**
   * @brief  Get the second temporary matrix for R2R fft. This matrix share
   * memory with temp2RealND.
   * @return Temporary real 2D matrix for axisymmetric code.
   */
  ...
  FftwComplexMatrix& getTempFftwX() {
    return mMatrixContainer.getMatrix<FftwComplexMatrix>(
        MatrixContainer::MatrixIdx::kTempFftwX);
  };
  /**
  ...
  FftwComplexMatrix& getTempFftwY() {
    return mMatrixContainer.getMatrix<FftwComplexMatrix>(
        MatrixContainer::MatrixIdx::kTempFftwY);
  };
  /**
  ...
  FftwComplexMatrix& getTempFftwZ() {
    return mMatrixContainer.getMatrix<FftwComplexMatrix>(
        MatrixContainer::MatrixIdx::kTempFftwZ);
  };
  /**
  ...
  FftwComplexMatrix& getTempFftwShift() {
    return mMatrixContainer.getMatrix<FftwComplexMatrix>(
        MatrixContainer::MatrixIdx::kTempFftwShift);
  };
   ...
   * @return Reference to the real matrix.
   */
  const RealMatrix& getRealMatrix(
      const MatrixContainer::MatrixIdx matrixIdx) const {
    return mMatrixContainer.getMatrix<RealMatrix>(matrixIdx);
  }
   ...
   * @return Reference to the index matrix.
   */
  const IndexMatrix& getIndexMatrix(
      const MatrixContainer::MatrixIdx matrixIdx) const {
    return mMatrixContainer.getMatrix<IndexMatrix>(matrixIdx);
  }
   ...
   * @return Pointer to raw data or nullptr if the matrix is not in the
   * container.
   */
  FloatComplex* getComplexData(const MatrixContainer::MatrixIdx matrixIdx,
                               const bool present = true) {
    return (present) ? mMatrixContainer.getMatrix<ComplexMatrix>(matrixIdx)
                           .getComplexData()
                     : nullptr;
  }
  ...
  float* getRealData(const MatrixContainer::MatrixIdx matrixIdx,
                     const bool present = true) {
    return (present)
               ? mMatrixContainer.getMatrix<RealMatrix>(matrixIdx).getData()
               : nullptr;
  }
  /**
  ...
  size_t* getIndexData(const MatrixContainer::MatrixIdx matrixIdx,
                       const bool present = true) {
    return (present)
               ? mMatrixContainer.getMatrix<IndexMatrix>(matrixIdx).getData()
               : nullptr;
  }
 ...
 private:
   ...
   * [Parameters::SimulationDimension, rho0ScalarFlag, bOnAScalarFlag,
   * c0ScalarFlag, alphaCoefScalarFlag]
   */
  using ComputeMainLoopImp = std::map<
      std::tuple<Parameters::SimulationDimension, bool, bool, bool, bool>,
      ComputeMainLoopFnc>;
