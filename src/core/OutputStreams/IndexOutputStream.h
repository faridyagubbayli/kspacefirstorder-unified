 ...
 *
 * @brief     The header file of the class responsible for storing output
 * quantities based on the index sensor mask into the output HDF5 file.
 *
 ...
 * @details Output stream for quantities sampled by an index sensor mask. This
 * class writes data to a single dataset in a root group of the HDF5 file
...
class IndexOutputStream : public BaseOutputStream {
 public:
   ...
   * @brief Constructor.
   *
   * @details The Constructor links the HDF5 dataset, source (sampled matrix),
   * sensor mask and the reduction operator together. The constructor DOES NOT
   * allocate memory because the size of the sensor mask is not known at the
   * time the instance of the class is being created.
   *
   * @param [in] file          - HDF5 file to write the output to.
   * @param [in] datasetName   - HDF5 dataset name. Index based sensor data is
   * stored in a single dataset.
   ...
   * grid points.
   */
  IndexOutputStream(Hdf5File& file, const MatrixName& datasetName,
