 ...
 * This class writes data into separated datasets (one per cuboid) under a given
 * dataset in the HDF5 file (time-series as well as aggregations).
 */
class CuboidOutputStream : public BaseOutputStream {
 public:
   ...
   * @param [in] groupName     - HDF5 group name. This group contains datasets
   * for particular cuboids.
   ...
   * grid points.
   */
  CuboidOutputStream(Hdf5File& file, const MatrixName& groupName,
 ...
 protected:
  ...
  /**
   * @brief  Create a new dataset for a given cuboid specified by the index
   * (order).
