...
BaseOutputStream::BaseOutputStream(Hdf5File& file,
    ...
    : mFile(file),
      ...
      mBufferSize(0),
      mStoreBuffer(bufferToReuse) {}  // end of BaseOutputStream
//----------------------------------------------------------------------------------------------------------------------

/**
 * Apply post-processing on the buffer. It supposes the elements are
 * independent.
 */
  ...
  switch (mReduceOp) {
    ...
    case ReduceOperator::kRms: {
      const float scalingCoeff =
          1.0f / (Parameters::getInstance().getNt() -
...
void BaseOutputStream::allocateMemory() {
  mStoreBuffer =
      (float*)_mm_malloc(mBufferSize * sizeof(float), kDataAlignment);
