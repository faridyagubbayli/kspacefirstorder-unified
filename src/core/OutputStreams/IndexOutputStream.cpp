...
IndexOutputStream::IndexOutputStream(Hdf5File& file,
    ...
    : BaseOutputStream(file, datasetName, sourceMatrix, reduceOp,
      ...
      mDataset(H5I_BADID),
      mSampledTimeStep(0) {}  // end of IndexOutputStream
//----------------------------------------------------------------------------------------------------------------------
...
void IndexOutputStream::create() {
  ...
  DimensionSizes datasetSize(
      nSampledElementsPerStep,
  ...
  mDataset = mFile.createDataset(
      mFile.getRootGroup(), mRootObjectName, datasetSize, chunkSize,
      Hdf5File::MatrixDataType::kFloat, params.getCompressionLevel());

  // Write dataset parameters
  mFile.writeMatrixDomainType(mFile.getRootGroup(), mRootObjectName,
                              Hdf5File::MatrixDomainType::kReal);
  mFile.writeMatrixDataType(mFile.getRootGroup(), mRootObjectName,
...
void IndexOutputStream::reopen() {
  ...
  if (mReduceOp == ReduceOperator::kNone) {  // Raw time series - just seek to
                                             // the right place in the dataset
    mSampledTimeStep =
        (params.getTimeIndex() < params.getSamplingStartTimeIndex())
            ? 0
            : (params.getTimeIndex() - params.getSamplingStartTimeIndex());
  } else {  // Aggregated quantities - reload data
    mSampledTimeStep = 0;
    // Read only if it is necessary (it is anything to read).
    if (params.getTimeIndex() > params.getSamplingStartTimeIndex()) {
      // Since there is only a single time step in the dataset, I can read the
      // whole dataset
      mFile.readCompleteDataset(mFile.getRootGroup(), mRootObjectName,
...
#pragma omp parallel for schedule(simd : static)
      ...
      flushBufferToFile();
      /* - for future use when offloading the sampling work to HDF5 - now it
      seem to be slower HDF5_File.WriteSensorbyMaskToHyperSlab(HDF5_DatasetId,
                                             Position,        // position in the
      dataset BufferSize,      // number of elements sampled
                                             SensorMask.GetRawData(), // Sensor
                                             SourceMatrix.GetDimensionSizes(),
      // Matrix dims SourceMatrix.GetRawData()); Position.Y++;
       */
      break;
    }  // case kNone
...
void IndexOutputStream::postProcess() {
  ...
  // When no reduction operator is applied, the data is flushed after every time
  // step
...
void IndexOutputStream::flushBufferToFile() {
  mFile.writeHyperSlab(mDataset, DimensionSizes(0, mSampledTimeStep, 0),
                       DimensionSizes(mBufferSize, 1, 1), mStoreBuffer);
  mSampledTimeStep++;
