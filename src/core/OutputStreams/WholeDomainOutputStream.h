 ...
 *
 * @brief     The header file of the class saving whole RealMatrix into the
 * output HDF5 file, e.g., p_max_all.
 *
 ...
 * @details Output stream for quantities sampled in the whole domain. The data
 * is stored in a single dataset (aggregated quantities only).
 */
class WholeDomainOutputStream : public BaseOutputStream {
 public:
  ...
  /**
   * @brief The constructor links the HDF5 dataset, source (sampled matrix) and
   * the reduction operator together.
   ...
   * grid points.
   */
  WholeDomainOutputStream(Hdf5File& file, const MatrixName& datasetName,
  ...
  /// Sample all grid points into a buffer and apply reduction, or flush to
  /// disk.
