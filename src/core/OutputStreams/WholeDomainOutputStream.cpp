 ...
 * the output HDF5 file, e.g., p_max_all.
 *
...
WholeDomainOutputStream::WholeDomainOutputStream(Hdf5File& file,
    ...
    : BaseOutputStream(file, datasetName, sourceMatrix, reduceOp,
                       bufferToReuse),
      mDataset(H5I_BADID),
      mSampledTimeStep(0) {}  // end of WholeDomainOutputStream
//----------------------------------------------------------------------------------------------------------------------
...
void WholeDomainOutputStream::create() {
  DimensionSizes chunkSize(mSourceMatrix.getDimensionSizes().nx,
                           mSourceMatrix.getDimensionSizes().ny, 1);

  // Create a dataset under the root group
  mDataset = mFile.createDataset(
      mFile.getRootGroup(), mRootObjectName, mSourceMatrix.getDimensionSizes(),
      chunkSize, Hdf5File::MatrixDataType::kFloat,
...
/**
 * Sample all grid points, line them up in the buffer an flush to the disk
 * unless a reduction operator is applied.
 */
void WholeDomainOutputStream::sample() {
  ...
  switch (mReduceOp) {
    case ReduceOperator::kNone: {
      ...
      // bigger datasets
      const DimensionSizes datasetPosition(
          0, 0, 0, mSampledTimeStep);  // 4D position in the dataset

      DimensionSizes cuboidSize(
          mSourceMatrix.getDimensionSizes());  // Size of the cuboid
      cuboidSize.nt = 1;

      mFile.writeCuboidToHyperSlab(
          mDataset, datasetPosition,
