 ...
 * @details Class storing all parameters of the simulation including the command
 * line parameters, simulation flags read from the input flag and several scalar
 * constants for homogeneous medium such as sound speed or acoustic density.
 *
...
class Parameters {
 public:
   ...
   * @details For normal medium, all three types are allowed, however, for the
   * AS medium the power law is not admissible. The values correspond to those
   ...
   * @details These flags are used as keys for the map holding parameter names
   * in the file.
   */
   ...
   * @details The routine also opens the input file and leave it open for
   further use.

   * @param   [in] argc - Number of command line parameters.
   * @param   [in] argv - Command line parameters.
   *
   * @throw   std::invalid_argument - If sampling is supposed to start out of
   ...
   *
   * @throw   ios:failure           - If the file cannot be open or is of a
   * wrong type or version.
   * @throw   std::invalid_argument - If some values are not correct or not
   * supported.
   ...
   * @param  [in] timer - A copy of timer measuring elapsed time from the
   * beginning
   * @return true - if the elapsed time in this leg has exceeded the specified
   * interval, or the simulation has executed a predefined number of time steps.
   */
   ...
   */
  const std::string& getNxHdf5Name() const {
    return sParameterHdf5Names[ParameterNameIdx::kNx];
  };
  /**
   * @brief  Get the HDF5 dataset name of Ny.
   * @return Hdf5 dataset name.
   */
  const std::string& getNyHdf5Name() const {
    return sParameterHdf5Names[ParameterNameIdx::kNy];
  };
  /**
   * @brief  Get the HDF5 dataset name of Nz.
   * @return Hdf5 dataset name.
   */
  const std::string& getNzHdf5Name() const {
    return sParameterHdf5Names[ParameterNameIdx::kNz];
  };
  ...
  SimulationDimension getSimulationDimension() const {
    return (isSimulation3D()) ? SimulationDimension::k3D
                              : SimulationDimension::k2D;
  };
   ...
   * @return Hdf5 dataset name.
   */
  const std::string& getTimeIndexHdf5Name() const {
    return sParameterHdf5Names[ParameterNameIdx::kTimeIndex];
  };

  /**
   * @brief Set simulation time step - should be used only when recovering from
   * checkpoint.
   ...
   * @details This routine exports wisdom of all FFTW plans, if compiled with
   * FFTW.
   */
  void exportWisdom();
  /**
   * @brief   Import FFTW wisdom from the file.
   * @details This routine imports wisdom of all FFTW plans, if compiled with
   * FFTW.
   */
  void importWisdom();
  ...
  /**
   * @brief  Get value of homogeneous medium density on staggered grid in x
   * direction.
  ...
  /**
   * @brief  Get value of homogeneous medium density on staggered grid in y
   * direction.
   * @return Staggered density.
   */
  ...
  /**
   * @brief  Get value of homogeneous medium density on staggered grid in z
   * direction.
   * @return Staggered density.
   */
 ...
 private:
   ...
   * @details In the case of cuboid corners sensor mask, the complete mask has
   * to be read and parsed. The data will be thrown away after calculating the
   * disk space and read again in the data loading phase. Fortunately, the
   * sensor mask is supposed to be small.
   */
  ...
  /// Container holding all the names of all possible parameters present in the
  /// HDF5 files.
  ...
  size_t mTimeIndex;
  /// How many timesteps to interruption. This index set after the simulation
  /// starts and decrements every time step.
